...
- **`/juridico`** — protegida, acessível pelo link "Jurídico" no cabeçalho (entre Taxa de Inadimplência % e Cadastro). Quadro Kanban por franquia (`app/juridico/page.js`):
  ...
  - **Editar/excluir card**: botões inline em cada card, abrem o mesmo modal de criação (pré-preenchido) ou removem direto (`DELETE /api/juridico/cards/:id`).
...
app/
  ...
  juridico/page.js         # Kanban Jurídico: etapas (colunas) + cards (vinculado a associado ou livre), drag and drop nativo
  ...
  controle-geral/page.js   # multi-franquia, Etapa 5: franquias (CRUD + usuário titular), bloquear/desbloquear usuário, resetar senha, "Meu perfil"
...
components/
  ...
  RequireSuperAdmin.js      # multi-franquia, Etapa 5: redireciona pro Dashboard quem não é SUPER_ADMIN (usado só em controle-geral/layout.js)
