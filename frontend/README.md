...
- **Modal de detalhe** (abre ao clicar na linha) — dados cadastrais, badges de Bloqueado/Jurídico, cobranças em aberto com link de pagamento, seção **Controle de bloqueios** (contador do ciclo via `GET .../bloqueios/contador`, badge de risco de perda de cashback a partir de 4 bloqueios, botão de reset com confirmação via `POST .../bloqueios/resetar`), histórico de negociação e campo de observação editável — logo abaixo do campo, mostra "Última observação: há Xd" (ou "hoje"/"há 1 dia") com a data completa entre parênteses, calculado a partir de `observacao_atualizada_em`, e o mesmo indicador amarelo de "sem contato recente" da tabela quando aplicável.
