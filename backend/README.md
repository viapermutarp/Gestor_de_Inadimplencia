...
> **Multi-franquia (em andamento, ver `docs/plano-multi-franquia.md`)**: `franquias` e `usuarios` já existem no schema, e todo model de negócio abaixo já tem `franquia_id` (`NOT NULL`, backfillado pra uma única franquia semente na migração `20260901130000_add_multi_franquia`). Todo `create`/leitura que precisa de um `franquia_id` hoje resolve a única franquia existente via `src/services/franquiaPadrao.service.js` (ponte temporária — ver seção 5.1 do plano para o incidente que motivou esse helper). O que ainda **não** existe: login usando `usuarios` (continua no admin único via `ADMIN_USER`/`ADMIN_PASSWORD` por enquanto) e o isolamento automático por franquia nas consultas (Prisma Client Extension) — os dados já estão rotulados, mas nada ainda filtra por eles. Ambos chegam nas próximas fases da mesma frente.
>
