...
> **Multi-franquia (em andamento, ver `docs/plano-multi-franquia.md`)**: `franquias` e `usuarios` já existem no schema, e todo model de negócio abaixo já tem `franquia_id` (`NOT NULL`, backfillado pra uma única franquia semente na migração `20260901130000_add_multi_franquia`). Todo `create`/leitura que precisa de um `franquia_id` hoje resolve a única franquia existente via `src/services/franquiaPadrao.service.js` (ponte temporária — ver seção 5.1 do plano para o incidente que motivou esse helper). Fase 2 (autenticação), Passo 1 já feito: `src/services/usuarios.service.js` semeia automaticamente 1 `Usuario` (`SUPER_ADMIN`, sem franquia) a partir de `ADMIN_USER`/`ADMIN_PASSWORD` na subida do servidor, se `usuarios` estiver vazia — idempotente, não trava a subida se falhar. O que ainda **não** existe: `POST /api/login` continua comparando direto contra `ADMIN_USER`/`ADMIN_PASSWORD` (Passo 2, ainda não implementado, vai trocar isso pra usar `Usuario` de verdade, com bcrypt e claims `franquiaId`/`papel` no token) e o isolamento automático por franquia nas consultas (Prisma Client Extension, Fase 3) — os dados já estão rotulados, mas nada ainda filtra por eles.
>
> - **franquias**: `id`, `nome` (editável), `ativo` (`false` bloqueia login de todos os usuários dela), `criado_em`
...
1. **Conta de serviço do Google** com a Drive API habilitada:
   ...
   - Defina a variável de ambiente `GOOGLE_SERVICE_ACCOUNT_JSON` com o **conteúdo desse arquivo**. Aceita tanto o JSON cru colado direto quanto o JSON inteiro em base64 (`base64 -w0 arquivo.json`) — mais seguro em plataformas onde colar um valor multi-linha é incômodo (ex.: EasyPanel). O backend tenta os dois formatos automaticamente.
...
2. **Pasta raiz do Drive**: configure via `PATCH /api/config/drive-pasta-raiz` (ou pela tela de Configurações do painel) — aceita tanto o id da pasta quanto o link completo (`https://drive.google.com/drive/folders/<id>`).
