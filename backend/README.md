...
- **Regressão — mesmos números de antes**: o dataset e as asserções já validadas em `test-status-ajustes.js` (17 pagamentos, 4 clientes) foram reproduzidos **verbatim** contra o caminho novo (Postgres local, populado via backfill) — `valor_total_faturado`, `valor_inadimplente`/`valor_adimplente`, as 7 faixas nos dois modos de `visao`, `criticos_90_dias`, `tipo_pendencia`, tolerância e exclusão por CPF/CNPJ deram **exatamente os mesmos valores** da rodada anterior (ex.: `valor_total_faturado: 3500`, `valor_inadimplente: 1700` no Grupo A; `faixas.ate_vencimento: 999` no Grupo B em `historico`) — confirma que a troca de fonte de dados não mudou nenhum resultado.
...
| `juridico` | `associados.em_juridico = true` |
| `critico` | Associado com **pelo menos 1** cobrança com **90+ dias de atraso** — mesmo critério/limiar (`LIMIAR_DIAS_CRITICO = 90`) já usado por `criticos_90_dias` no card, **respeitando o `visao` selecionado** (`aberto`: só considera `OVERDUE`; `historico`: considera a classificação por data com tolerância, igual a `computarFaixasECriticos`). |
...
- Regressão: sem nenhum parâmetro novo, `valor_total_faturado`/`criticos_90_dias` idênticos ao comportamento anterior a este ajuste.
...
- **`tipo_inadimplente`**: `ativo` (exclui só o associado jurídico do dataset), `juridico` (só ele), `critico` em `visao=aberto` (só quem tem cobrança `OVERDUE` com 90+ dias — outro associado do dataset, com atraso grande mas só via pagamento já recebido, corretamente **não** entra nesse modo), `critico` em `visao=historico` (esse outro associado passa a entrar, via classificação por data/tolerância — mesma função já usada por `criticos_90_dias`), `juridico,critico` juntos no mesmo associado sem duplicar valor, valor inválido → `400`.
