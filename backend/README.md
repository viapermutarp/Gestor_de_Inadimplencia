...
- **associados**: `id`, `franquia_id` (FK), `cpf_cnpj` (único, indexado), `nome`, `nome_asaas` (nullable — nome exatamente como cadastrado no Asaas, verbatim, sem parsing/tratamento, incluindo o prefixo numérico que o Asaas usa; AJUSTE 9 — ver seção própria abaixo; não confundir com `nome` acima, que segue sendo o nome tratado/exibido no título do modal), `telefone`, `email`, `em_negociacao`, `observacao`, `observacao_atualizada_em` (data/hora da última mudança de valor de `observacao`, só via `PATCH .../negociacao` — ver seção de endpoints), `bloqueado`, `em_juridico`, `ciclo_resetado_em` (marco usado pelo contador de bloqueios), `criado_em`, `atualizado_em`. **AJUSTE 19** — 21 campos novos, todos nullable, exclusivos do Cadastro (`POST /api/cadastros`) e da importação de CSV (`POST /api/associados/importar/aplicar`) — **nunca** escritos por `POST /api/sync` nem pelo sync/backfill do Asaas, pra não haver conflito de quem escreve o quê: identificação (`tipo_pessoa` `"PF"`\|`"PJ"`, `razao_social`, `nome_fantasia`), endereço (`cep`, `endereco`, `numero`, `complemento`, `bairro`, `cidade`, `uf`), contato (`contato_nome`, `celular`, `email_cadastro` — distintos dos legados `telefone`/`email`, que continuam só do sync), faturamento (`descricao_servico`, `valor_entrada`, `data_entrada`, `numero_parcelas`, `valor_parcela`, `valor_total`, `data_vencimento`, `desconto_parcela`, `observacoes_cadastro`). Índice extra em `email_cadastro`. Ver seção "AJUSTE 19" abaixo para o desenho completo.
...
| GET | `/api/associados/:cpf_cnpj` | Detalhe de um associado, com todas as cobranças e `historico` — as mudanças de `em_negociacao`, `bloqueado` e `em_juridico` juntas num só array (`{ id, associado_id, campo, status_anterior, status_novo, alterado_em }`), mais recente primeiro. Também traz `nome_asaas` (AJUSTE 9 — pode ser `null` até o backfill/sync popular) e, desde o AJUSTE 19, os 21 campos novos do Cadastro (`tipo_pessoa`, `razao_social`, `cep`, `valor_total` etc. — ver "Modelo de dados" acima). **AJUSTE 19**: exige recurso `dashboard` **OU** `associados` (qualquer um dos dois libera — reaproveitado tanto pelo Dashboard quanto pela aba nova "Associados", sem duplicar o endpoint; ver `exigirRecurso.js`). Ver "Histórico unificado de status" e "AJUSTE 9" abaixo. |
...
| PATCH | `/api/associados/:cpf_cnpj/juridico` | Body `{ "em_juridico": bool }`. Atualiza o campo e grava uma linha em `historico_status_associado` (`campo: "em_juridico"`). **AJUSTE 10**: na mesma transação, também cria ou exclui automaticamente o(s) card(s) vinculados a este associado no Kanban Jurídico — ver seção "AJUSTE 10" abaixo. Resposta traz `juridico: { evento, ... }` descrevendo o que aconteceu do lado do Kanban. |
...
| GET | `/api/associados/registro` | **AJUSTE 19.** Exige recurso `associados` (não `dashboard`). Listagem enxuta da carteira cadastral pra aba "Associados" — diferente de `GET /api/associados`: não filtra por cobrança em aberto, não ordena por atraso, não traz cobranças/histórico. **AJUSTE 20**: só lista quem **tem cadastro** (pelo menos um dos 22 campos do AJUSTE 19 preenchido — ver seção "AJUSTE 20" abaixo). **Paginada** (`page`, `limit`, mesmo padrão 1/100). `busca` pesquisa por nome, razão social, nome fantasia, cpf_cnpj, email OU email_cadastro (contains, case-insensitive, exceto cpf_cnpj) — combinado com o filtro de "tem cadastro" (E, não OU). Resposta: `{ dados, paginacao }`. |
| POST | `/api/associados/importar` | **AJUSTE 19.** Exige recurso `associados`. `multipart/form-data`, campo `arquivo` (CSV, exportação de contatos do Bling — ver seção "AJUSTE 19" abaixo pro mapeamento de colunas). **Só leitura** — não grava nada. Detecta charset (UTF-8 com fallback Latin-1) e delimitador (`;` ou `,`) automaticamente. Resposta: `{ total_linhas, delimitador_detectado, novos, conflitos, erros }` — `conflitos[].atual` traz o que já está no banco, os demais campos do item são o que veio na planilha, pro frontend comparar lado a lado. `400` se o CSV não tiver uma coluna de CNPJ/CPF reconhecível. |
| POST | `/api/associados/importar/aplicar` | **AJUSTE 19.** Exige recurso `associados`. Body `{ "novos": [...], "decisoes": [...] }` — `novos` é o array devolvido por `/importar` (sempre aplicado, sem decisão); `decisoes` é o array de `conflitos` de `/importar` com um campo `"acao": "atualizar"\|"pular"` adicionado por item (decisão individual, nunca em massa). Idempotente (`upsert`). Cada linha processada de forma independente — uma falha não derruba as demais. Resposta: `{ criados, atualizados, pulados, erros: [{ linha, cpf_cnpj, motivo }] }`. |
| DELETE | `/api/associados/:cpf_cnpj/cadastro` | **AJUSTE 20.** Exige recurso `associados`. Limpa (`null`) só os 22 campos de cadastro do AJUSTE 19 — nunca os legados (`nome`, `telefone`, `email`, `em_negociacao`, `bloqueado`, `em_juridico` etc.) nem nenhuma outra tabela. `404` se o CPF/CNPJ não existir **nesta franquia** (isolamento automático via `req.prisma`). Resposta: `{ cpf_cnpj, cadastro_excluido: true }`. |
| POST | `/api/associados/cadastro/excluir-lote` | **AJUSTE 20.** Exige recurso `associados`. Body `{ "cpf_cnpjs": ["...", "..."] }`. Mesma limpeza do endpoint individual, aplicada a cada item de forma independente — um CPF/CNPJ inexistente (ou de outra franquia) não derruba o resto do lote, só entra em `nao_encontrados`; um que já está sem cadastro é achado normalmente e conta como `excluidos` (idempotente). `400` se `cpf_cnpjs` vier ausente, vazio ou não for array. Resposta: `{ total_solicitados, excluidos, nao_encontrados }`. |
...
| POST | `/api/juridico/associados/:cpf_cnpj/documentos` | **AJUSTE 11.** `multipart/form-data`, campo `arquivo` (obrigatório) + `descricao` (opcional). Upload de um documento anexado ao associado — ver seção "Documentos anexados ao Jurídico" abaixo. `404` se o CPF/CNPJ não corresponder a um associado desta franquia. `400` se o arquivo não passar na validação (tipo não permitido, mimetype declarado não bate com a extensão, assinatura dos bytes não bate com o tipo, ou tamanho acima do limite). |
| GET | `/api/juridico/associados/:cpf_cnpj/documentos` | **AJUSTE 11.** Lista todos os documentos do associado (`[{ id, cpf_cnpj, nome_original, tipo_mime, tamanho_bytes, enviado_por, enviado_por_nome, descricao, criado_em }]`), mais recente primeiro — independente de existir card aberto no momento (ver docblock do model `DocumentoJuridico`). `404` se o associado não existir nesta franquia. |
| GET | `/api/juridico/documentos/:id/download` | **AJUSTE 11.** Baixa o arquivo em si (streaming do disco, com `Content-Type`/`Content-Disposition` corretos). `404` se o documento não existir (inclusive de outra franquia) ou se o arquivo não for encontrado em disco (ver aviso sobre volume persistente na seção "Deploy no EasyPanel"). |
| GET | `/api/juridico/documentos/:id/preview` | **AJUSTE 12.** Visualização **inline** (sem download) — ver seção "AJUSTE 12" abaixo. PDF/JPG/PNG: stream do arquivo original com `Content-Disposition: inline`. DOCX/XLSX: convertidos pra HTML **sanitizado** na hora (`mammoth`/`sheet_to_html`), resposta `{ "tipo": "html", "html": "..." }`. `404` mesmo padrão do download; `422` com mensagem clara se a conversão falhar (arquivo corrompido/formato inesperado). |
| DELETE | `/api/juridico/documentos/:id` | **AJUSTE 11.** Apaga o registro **e** o arquivo em disco. Independente de card — não há relação nenhuma com `cards_juridico` aqui (ver docblock do model). `404` se não existir nesta franquia. |

### Restrição de telas por franquia

Cada usuário passa a ter uma lista configurável (`usuarios.recursos_permitidos`, array de string) de quais telas ele pode acessar — `dashboard`, `associados`, `inadimplencia`, `cadastro`, `contratos`, `juridico`, `configuracoes` (chaves canônicas em `src/config/recursos.js`, reaproveitadas por `franquias.controller.js`/`usuarios.controller.js` na validação e por `middleware/exigirRecurso.js` na aplicação). **"Controle Geral" nunca entra nessa lista** — não é um recurso restringível, continua sendo SUPER_ADMIN only sem exceção, e não depende de usuário/franquia nenhuma.

> **AJUSTE 19** — `associados` é um recurso **próprio**, não reaproveita `dashboard` (decisão explícita, pedida ao usuário): um usuário só com `dashboard` não enxerga a aba "Associados" (e vice-versa). A única rota compartilhada entre as duas telas é o detalhe `GET /api/associados/:cpf_cnpj`, que passou a aceitar **qualquer uma** das duas — ver `exigirRecurso` logo abaixo.
...
- **AJUSTE 19** — `exigirRecurso` também aceita um **array de chaves** (`exigirRecurso(['dashboard', 'associados'])`): libera a rota se o usuário tiver **qualquer uma** delas (OR, não AND). Único uso hoje: `GET /api/associados/:cpf_cnpj` (ver acima). Uso normal com uma chave só (string) continua idêntico.

Mapeamento de rotas → recurso: `associados.routes.js` (Dashboard, inclui o toggle `em_juridico` do próprio associado — não confundir com a aba "Jurídico"/Kanban) → `dashboard`; `/associados/registro`, `/associados/importar`, `/associados/importar/aplicar`, `/associados/:cpf_cnpj/cadastro` (DELETE, AJUSTE 20), `/associados/cadastro/excluir-lote` (AJUSTE 20) (mesmo arquivo, AJUSTE 19) → `associados` (recurso próprio); `/associados/:cpf_cnpj` (detalhe, compartilhado) → `dashboard` OU `associados`; `inadimplencia.routes.js` → `inadimplencia`; `cadastros.routes.js` → `cadastro`; `contratos.routes.js` → `contratos`; `config.routes.js` (toda a tela Configurações) → `configuracoes`; `juridico.routes.js` → `juridico`; `sync.routes.js`: só `POST /api/sync/atualizar` (botão "Atualizar" do Dashboard, sessão JWT) exige `dashboard` — `POST /api/sync` em si roda por API key, sempre isento.

**Rotas literais antes das paramétricas** — `router.get('/associados/registro', ...)` precisa vir **antes** de `router.get('/associados/:cpf_cnpj', ...)` no mesmo arquivo (mesmo padrão pras 2 rotas de importação), senão o Express casaria `"registro"`/`"importar"` com o parâmetro `:cpf_cnpj` primeiro — ver `associados.routes.js`.
...
Antes deste ajuste, `emJuridico` no associado e o Kanban Jurídico eram dois mundos sem nenhuma ligação automática: marcar um associado como "Jurídico" no Dashboard só servia pra excluí-lo da Régua de Cobrança e pros filtros/cross-reference (`GET /api/associados?em_juridico=`, `/api/inadimplencia/resumo?em_juridico=`) — vincular um card a ele no quadro Jurídico era sempre um passo manual separado, e nada removia o card se o associado fosse desmarcado depois. Este ajuste conecta os dois lados, sempre pelo `PATCH /api/associados/:cpf_cnpj/juridico` (nunca pelo caminho inverso — ver nota no fim desta seção):

- **Marcar `em_juridico: true`**: busca um card já **aberto** vinculado a esse associado (`associado_id`) — "aberto" aqui é qualquer card não excluído, em qualquer etapa (não existe hoje uma coluna "arquivado" que precisasse ficar de fora dessa checagem). Se já existir, **não cria outro** — a resposta traz `juridico: { evento: "card_ja_existente", card_id, etapa_id, etapa_nome }`, pro frontend avisar o usuário e linkar direto pro card (ver frontend, seção própria). Se não existir, cria um card novo vinculado (modo "Vincular associado", igual um card criado manualmente pela tela) na **primeira etapa** do quadro — a de menor `ordem` cadastrada na franquia, não uma etapa com nome fixo tipo "Iniciar Cobrança" (o nome da primeira coluna é livre, cada franquia pode ter renomeado a dela) — com nome/CPF-CNPJ/telefone/valor em aberto vindos sempre ao vivo da relação, exatamente como qualquer outro card vinculado (ver "Kanban Jurídico" acima). Resposta: `juridico: { evento: "card_criado", card_id, etapa_id, etapa_nome }`. Se a franquia ainda não tem **nenhuma** etapa cadastrada (quadro vazio), não há onde colocar o card — não cria nada, não falha o `PATCH`, só devolve `juridico: { evento: "sem_etapas" }` pro frontend avisar que precisa criar uma coluna primeiro.
- **Desmarcar `em_juridico: false`**: exclui (hard delete, mesmo comportamento de `DELETE /api/juridico/cards/:id`) **todos** os cards vinculados a esse associado — pode haver mais de um (só possível antes deste ajuste existir, ou por vínculo manual duplicado feito direto na tela); exclui todos, não só o mais recente. A etapa atual de cada card não importa — mesmo um card já movido manualmente pra uma etapa mais avançada (ex.: "Processos em Andamento") é excluído do mesmo jeito, só o vínculo com o associado decide. Cada exclusão grava o evento `"exclusao"` em `historico_card_juridico` **antes** do delete de verdade (mesma regra de sempre — o log não tem FK pro card, então o registro de que aquele card existiu e o que aconteceu nele não se perde, só o card em si some do quadro) e reindexa `ordem` dos cards restantes na etapa de origem. Resposta: `juridico: { evento: "cards_excluidos", quantidade }` (`quantidade` pode ser `0`, se não havia nenhum card vinculado).
- **Criação/exclusão automática rodam na MESMA transação** da mudança de `em_juridico`/gravação do histórico de status (`$transaction` em `atualizarJuridico`, `associados.controller.js`) — se qualquer parte falhar, nada é commitado (nem o `em_juridico` novo, nem o card criado/excluído). A lógica em si (achar card aberto, criar na primeira etapa, excluir todos) foi extraída pra `src/services/juridicoCard.service.js`, compartilhada com `juridico.controller.js` (que usa as mesmas funções pras mutações manuais).
- **Idempotente, de propósito**: os dois passos acima rodam **sempre** que o endpoint é chamado com aquele valor — não só numa transição real de `false` pra `true` (ou vice-versa). Isso cobre dois casos: (a) auto-recuperação — um associado que já estava `em_juridico: true` mas cujo card sumiu por qualquer motivo volta a ganhar um card no próximo `PATCH .../juridico` com `true` (mesmo que o valor enviado seja "igual" ao que já estava salvo); (b) toggles rápidos em sequência (marcar e desmarcar antes de qualquer sync/refresh) nunca deixam card órfão nem duplicado — cada chamada lê o estado real do banco **dentro da própria transação**, nunca um estado em cache do frontend, e a segunda chamada só começa depois que a primeira já commitou (o toggle da tela desabilita um novo clique enquanto a chamada anterior está em voo).
- **Só numa direção**: editar um card manualmente pela tela do Jurídico (responsável, prazo, descrição, observações, mover de etapa) **nunca** mexe em `emJuridico` do associado — a ligação automática existe só de Dashboard pra Jurídico, nunca o inverso. Um card vinculado pode inclusive ser excluído manualmente pela tela do Jurídico (`DELETE /api/juridico/cards/:id`) sem que isso desmarque `em_juridico` do associado — nesse caso, o próximo `PATCH .../juridico` com `true` recria o card (auto-recuperação, ver item acima); se ninguém chamar o endpoint de novo, o associado fica `em_juridico: true` sem card nenhum até isso acontecer.

### AJUSTE 11 — Documentos anexados ao Jurídico

Cada associado passa a poder ter documentos anexados (contratos assinados, comprovantes, procurações etc.), visíveis na aba "Documentos" do card do Kanban Jurídico vinculado a ele. Endpoints: ver tabela acima (`POST`/`GET .../documentos`, `GET .../download`, `DELETE`).

- **Ligado por `cpf_cnpj`, não por `card_id` — de propósito.** O model `DocumentoJuridico` não tem nenhuma coluna/FK apontando pra `CardJuridico`. Isso é o que garante que os documentos **sobrevivem** à exclusão do card: desde o AJUSTE 10, desmarcar `em_juridico` no Dashboard **exclui de verdade** (hard delete) o(s) card(s) vinculado(s) — sem essa decisão de design, todo documento anexado seria perdido junto. Documentos podem existir mesmo sem nenhum card aberto no momento (ex.: associado desmarcado do Jurídico depois de já ter documentos enviados) — o card é só uma "janela" de visualização; se um novo card for criado depois pro mesmo associado (manualmente ou pela auto-criação do AJUSTE 10), os documentos já enviados **reaparecem automaticamente** na aba Documentos dele, sem nenhuma ação extra.
  - **Testado de ponta a ponta contra Postgres real** (`scripts/test-ajuste11-documentos.js`, 27/27 — servidor Express real via `node src/server.js`, banco Postgres real, chamadas HTTP reais via `fetch`, nada mockado): fixture de franquia/usuário/associado/etapa gravada direto via Prisma → login real (`POST /api/login`) → `PATCH .../juridico` com `true` cria o card (confirmado no banco) → upload de um PDF real (confirmado no banco **e** em disco) → `PATCH .../juridico` com `false` **exclui de verdade** o card (confirmado: `findUnique` retorna `null`, evento `"exclusao"` gravado em `historico_card_juridico`) → **documento continua existindo, no banco e em disco, com o card já excluído** → `GET .../documentos` sem nenhum card aberto ainda lista o documento → `PATCH .../juridico` com `true` de novo cria um **card novo, com ID diferente** do original → **o documento aparece de novo na aba Documentos do card novo, sem nenhum re-upload** → download baixa um arquivo byte-idêntico ao original → exclusão do documento remove registro e arquivo. Script não commitado no repositório (mesmo padrão dos demais `test-*.js` referenciados neste README — usados uma vez pra validar e descartados, não fazem parte do código de produção); resultado completo (27 asserções, todas `OK`) fica registrado aqui.
- **Armazenamento em disco**, organizado por CPF/CNPJ normalizado (só dígitos): `<JURIDICO_UPLOADS_DIR>/<cpf_cnpj só dígitos>/<timestamp>-<nome sanitizado>`. `JURIDICO_UPLOADS_DIR` (ver `src/config/env.js` e `.env.example`) — se não definida, usa `<raiz do backend>/uploads/juridico`. **Em produção isso PRECISA ser um volume persistente** — ver aviso já existente mais acima nesta seção sobre o filesystem do container não ser garantido persistente entre deploys no EasyPanel, e o passo 6 novo em "Deploy no EasyPanel" abaixo.
  - **⚠️ PENDENTE — NÃO CONFIRMADO EM PRODUÇÃO.** O volume persistente do `docker-compose.yml` (deploy local/self-hosted) foi configurado e é só configuração declarativa — não tive como testá-lo de fato (exigiria rodar `docker compose up`/`down` e confirmar que o arquivo sobrevive, o que não foi feito nesta sessão). No EasyPanel especificamente, ninguém configurou o volume ainda — é uma ação manual no painel deles (passo 6 abaixo) que só o time da Via Permuta pode fazer, já que exige acesso ao painel do EasyPanel, que eu não tenho. **Enquanto esse passo não for feito, qualquer documento enviado em produção via EasyPanel será perdido no próximo deploy.** Isto não deve ser tratado como resolvido até alguém (a) configurar o volume na aba Mounts/Volumes do serviço da API no EasyPanel e (b) confirmar na prática — subindo um documento de teste, disparando um novo deploy, e verificando que o documento continua baixável depois.
- **Validação de arquivo em duas camadas, sempre as duas** (`src/services/armazenamentoDocumentos.service.js`): (1) a extensão do nome **e** o `mimetype` declarado no `multipart/form-data` precisam bater com um tipo da allowlist (hoje: PDF, DOCX, XLSX, JPG/JPEG, PNG — fácil de expandir, é só uma entrada nova em `TIPOS_PERMITIDOS`); (2) os primeiros bytes do arquivo (assinatura/"magic bytes") precisam corresponder ao tipo de verdade — é isso que bloqueia um executável renomeado pra `.pdf`: o conteúdo real não começa com `%PDF-`, então é rejeitado mesmo que nome e `Content-Type` do upload mintam. Tamanho máximo configurável via `JURIDICO_UPLOAD_MAX_BYTES` (default 20MB) — validado tanto pelo `multer` (limite de upload) quanto de novo dentro do serviço.
  - **Decisão consciente: sem a lib `file-type`.** As versões atuais dela são ESM-only e quebrariam o `require()` deste projeto (CommonJS). A checagem de assinatura feita à mão não distingue DOCX de XLSX com 100% de certeza usando só os 4 primeiros bytes (os dois são arquivos ZIP, `PK\x03\x04`) — complementada por uma heurística (não um parser de ZIP completo): procurar pelas entradas típicas `word/`/`xl/` em algum lugar dos bytes do arquivo. O que isso garante com certeza absoluta é a ameaça citada no escopo do pedido — "rejeitar tipos executáveis mesmo que renomeados" — já que um executável tem assinatura completamente diferente (`MZ`, ELF) e nunca passa em nenhuma das checagens.
- **Segurança contra path traversal**: nome de arquivo sanitizado (`path.basename` + allowlist de caracteres seguros, sem diacríticos) antes de gravar, e todo caminho relativo gravado no banco é resolvido de novo contra o diretório raiz antes de qualquer leitura/escrita/exclusão (`resolverCaminhoSeguro`, defesa em profundidade) — um `caminho_arquivo` malicioso tipo `../../etc/passwd` nunca resolve fora de `JURIDICO_UPLOADS_DIR`.
- **Isolamento por franquia**: `documento_juridico` está no `ESCOPO_DIRETO` da Prisma Client Extension de isolamento (mesmo padrão de `EtapaJuridico`/`CardJuridico`/`HistoricoCardJuridico`) — `franquia_id` direto na própria tabela.
- **Acesso**: qualquer usuário com a tela Jurídico liberada (recurso `juridico`) pode enviar/ver/baixar/excluir documentos — nenhuma permissão nova, mesma checagem `exigirRecurso('juridico')` de todo o resto do Jurídico.
- **`multer` com `memoryStorage`, não `diskStorage`**, de propósito: o arquivo só é gravado em disco **depois** de passar pelas duas camadas de validação — com `diskStorage` ele já teria sido escrito antes da validação rodar.

### AJUSTE 12 — Correção de encoding do nome do arquivo + Visualização inline de documentos

Dois problemas separados no mesmo pedido: (1) nomes de arquivo com acento chegando corrompidos no banco (ex.: `"CartÃ£o.pdf"` em vez de `"Cartão.pdf"`); (2) nova visualização inline dos documentos do Jurídico (sem download), direto dentro do modal do card.

**Bug do encoding — causa raiz confirmada por reprodução, não só suposição.** `multer` roda em cima do `busboy`, que decodifica o cabeçalho `Content-Disposition: ...; filename="..."` do `multipart/form-data` como **Latin-1 por padrão** — mesmo quando o navegador manda UTF-8 de verdade (todo navegador manda; a RFC 7578 não define charset explícito pra esse campo, então bibliotecas mais antigas assumem Latin-1). Cada caractere multi-byte UTF-8 (ex.: `"ã"` = `0xC3 0xA3`) acaba lido como 2 caracteres Latin-1 separados (`Ã` + `£`), daí a corrupção. Reproduzido isoladamente com `multer@1.4.5-lts.1` + `express`, tanto via `curl -F` quanto via `fetch`/`FormData` nativos do Node (mesmo mecanismo do frontend) — os dois geraram o mesmo `req.file.originalname` corrompido, confirmando a causa antes de qualquer correção ser aplicada (por pedido explícito do escopo).

- **Correção**: `Buffer.from(originalname, 'latin1').toString('utf8')`, aplicada uma única vez em `uploadMiddleware` (`juridicoDocumentos.controller.js`), logo depois que o `multer` termina de montar `req.file` — todo consumidor downstream (validação, nome salvo em disco, nome gravado no banco) já recebe o nome corrigido. Segura pra nomes 100% ASCII (sem acentuação): Latin-1 e UTF-8 são idênticos nesse intervalo, o round-trip não altera nada.
- **Documentos já enviados ANTES da correção continuam com o nome corrompido no banco** — a correção só vale pra uploads novos, não reprocessa nada existente automaticamente. Script pontual pra levantar o estrago: `scripts/diagnostico-nomes-documentos-corrompidos.js` — roda em modo **dry-run por padrão** (só lista, não altera nada), heurística de detecção documentada no próprio arquivo (reconverte Latin-1→UTF-8 e só considera "corrompido" se o resultado for diferente do original **e** não gerar caractere de substituição `�` — evita falso positivo em nome legitimamente acentuado). `--corrigir` aplica o `UPDATE` nos que a heurística identificou; `--franquia=<id>` filtra uma franquia só. **Precisa rodar dentro do ambiente de produção** (mesma `DATABASE_URL` real — ver docblock do script pra como acessar um shell no EasyPanel, mesmo padrão de `scripts/diagnostico-ajuste8.js`).
  - **⚠️ PENDENTE — não executado contra o banco de produção.** Não tenho acesso direto ao banco/shell de produção pra rodar este script e contar quantos documentos reais estão afetados. **Recomendação (pedida explicitamente no escopo): rodar o script em modo dry-run primeiro e ver a contagem antes de decidir o quê fazer** — se forem poucos, o mais simples é reenviar manualmente os arquivos afetados (upload de novo, com o nome já correto); rodar `--corrigir` é mais arriscado porque reconverter um nome que já está corrompido nem sempre volta ao original perfeito (a heurística evita falsos positivos, mas não é uma prova matemática de que cada resultado bate 100% com o nome original de quem enviou). Isto não deve ser tratado como resolvido até alguém com acesso ao ambiente de produção rodar o script e reportar a contagem.
  - Verificado com `node --check` e testado isoladamente (5 casos, incluindo um nome legitimamente acentuado — "Ângela Souza.pdf" — pra confirmar que a heurística não o marca como corrompido); rodado também contra a base de teste usada na verificação end-to-end abaixo (0 documentos corrompidos, como esperado — todos os uploads dali já passaram pela correção).

**Visualização inline (`GET /api/juridico/documentos/:id/preview`, `src/services/previewDocumento.service.js`)** — abre o documento dentro do próprio modal do card, sem download:

- **PDF e imagem (JPG/PNG)**: stream do arquivo **original**, sem conversão nenhuma — só `Content-Disposition: inline` em vez de `attachment` (`valorContentDisposition` ganhou um segundo parâmetro pra isso).
- **DOCX**: convertido pra HTML na hora via `mammoth` (`mammoth.convertToHtml`).
- **XLSX**: convertido pra tabela HTML na hora via `xlsx`/SheetJS (`XLSX.utils.sheet_to_html`, só a primeira aba).
- **Sem cache** de propósito (decisão consciente do escopo do pedido — volume baixo de documentos não justifica a complexidade de invalidar cache no reenvio); se ficar perceptivelmente lento em uso real, cachear o HTML convertido (invalidando quando o documento for reenviado) fica como melhoria futura, não bloqueante.
- **Falha de conversão nunca vira `500`** — sempre `422` com mensagem clara (`ErroPreviewDocumento`), pro frontend cair no estado "Não foi possível gerar visualização, baixe o arquivo" em vez de quebrar. Cobre tanto DOCX/XLSX genuinamente corrompidos quanto — achado **real em teste, não hipotético** — o fato de `XLSX.read()` do SheetJS **não** lançar erro pra bytes que não são um XLSX de verdade: ele tenta interpretar qualquer coisa como CSV por padrão, silenciosamente virando uma "planilha" com lixo dentro em vez de falhar. Corrigido checando a assinatura ZIP (`PK\x03\x04`, mesma checagem de `armazenamentoDocumentos.service.js`) antes de chamar `XLSX.read()` — defesa em profundidade pro caso de um arquivo corromper **depois** de já salvo (disco, ou justamente o cenário do volume persistente do EasyPanel ainda não confirmado, ver AJUSTE 11 acima).
- **Sanitização obrigatória do HTML convertido** (`sanitize-html`, allowlist explícita — nunca blocklist — de tags/atributos/esquemas de URL; `data:` só permitido em `<img>`, já que o `mammoth` embute imagens do docx como data URI; todo `<a>` sobrevivente ganha `rel="noopener noreferrer" target="_blank"`) — o HTML que sai do `mammoth`/`sheet_to_html` vem do **conteúdo de um arquivo enviado por usuário**, nunca é seguro jogar direto num `dangerouslySetInnerHTML`. Sanitizado no **backend** (fonte única da verdade); o frontend sanitiza de novo com `DOMPurify` antes de renderizar, como camada extra (ver README do frontend), mas o HTML que sai desta API já é seguro por si só.
  - **Testado com um payload adversarial real, não só teoricamente**: gerado um `.docx` de verdade (via `python-docx`, manipulando o XML diretamente) com um hyperlink apontando pra `javascript:alert(1)` e convertido pelo **`mammoth` de verdade** (não um mock) — o `href` malicioso não sobrevive à sanitização (o texto do link continua, só o `href` é removido), confirmado no HTML de saída. Testado também, direto contra `sanitizeHtml()`, um payload cru com `<script>`, `onclick`/`onerror`/`onload`, `href="javascript:"`/`"vbscript:"`, `<iframe>`, `<form>`/`<input>`, `<svg onload>` e `<style>` — todos removidos; link `https://` legítimo e `<img src="data:image/...">` (caso real do `mammoth`) preservados corretamente, com `rel`/`target` adicionados ao link.
- **Verificação end-to-end contra Postgres real** (mesma metodologia já usada no AJUSTE 11 — servidor Express real via `node src/server.js`, banco Postgres real, chamadas HTTP reais via `curl`, nada mockado): login real → upload real de PDF, JPG (nome acentuado — confirma a correção do encoding também por HTTP de verdade, não só no reprodutor isolado), DOCX válido, DOCX malicioso (hyperlink `javascript:`) e XLSX válido → `GET .../preview` de cada um: PDF/JPG voltaram `200` com `Content-Type`/`Content-Disposition: inline` corretos e os bytes certos; DOCX/XLSX válidos voltaram `200` com o HTML esperado; DOCX malicioso voltou `200` com HTML sem `javascript:` em lugar nenhum; simulado um DOCX e um XLSX corrompidos **depois** de já salvos em disco (sobrescrevendo o arquivo salvo com lixo) — os dois voltaram `422` com mensagem clara, não `500`; documento inexistente voltou `404`.

**Frontend**: botão "Visualizar" ao lado de "Baixar" em cada documento (`DocumentosCard`, `app/juridico/page.js`) — abre/fecha uma área de preview dentro do próprio card (não em nova aba): PDF via `<iframe>`, imagem via `<img>`, DOCX/XLSX via o HTML sanitizado (backend + `DOMPurify` de novo no cliente) dentro de um container com scroll. Estado de erro (`422`/qualquer falha) cai no texto "Não foi possível gerar visualização, baixe o arquivo" com um atalho pra "Baixar" ali mesmo. PDF/imagem chegam como blob URL autenticado (mesmo truque do download, sem disparar "salvar como") — revogado (`URL.revokeObjectURL`) sempre que o preview é fechado, trocado pra outro documento, ou o componente desmonta (fechar o modal do card).
...
**Fluxo, passo a passo:**
1. **AJUSTE 19** — antes de tudo, faz `upsert` do `Associado` correspondente (chave: `"CNPJ/CPF"` do payload) **e** cria a linha em `cadastros_enviados` com `status: "enviado"`, **na mesma transação** (`req.prisma.$transaction`, forma callback) — se qualquer um dos dois falhar, nenhum dos dois é salvo. Ver mapeamento completo de campos e o motivo de rodar **antes** do webhook (não depois) na seção "AJUSTE 19" abaixo.
...
### Classificação histórica de inadimplência (data de pagamento) — usada só em `faixas`/`valor_criticos`
...
| `valor_criticos` | Soma do `value` dos pagamentos com 90 dias de atraso ou mais, seguindo o mesmo modo (`aberto`/`historico`) de `faixas` — ver seção "Faixas de atraso: modo aberto x histórico". Métrica independente das faixas — um pagamento pode entrar tanto em `valor_criticos` quanto na faixa `51_100` (ex.: 90-99 dias caem na faixa `51_100` **e** em `valor_criticos`). |
...
### AJUSTE 14 — Tabela local sincronizada via webhook do Asaas para Taxa de Inadimplência

**Objetivo**: `GET /api/inadimplencia/resumo` e `/evolucao-mensal` consultavam a API do Asaas **ao vivo** a cada troca de filtro — paginando `/payments` e resolvendo cliente por cliente via `/customers/{id}`. Em franquias com muito histórico isso ficava lento (e sujeito a rate limit do Asaas) a cada clique num filtro. Este ajuste elimina as chamadas ao vivo: os dois endpoints passam a ler de uma tabela local no Postgres (`pagamentos_asaas`), **sincronizada em tempo real via webhook do Asaas**, com um backfill inicial (histórico) e uma reconciliação periódica (rede de segurança) como apoio. **A lógica de classificação em si não mudou nem uma linha** — só a fonte dos dados.

**Tabela nova — `pagamentos_asaas`** (model `PagamentoAsaas`, migração `20260910183034_add_pagamentos_asaas`): `id` (o próprio `pay_...` do Asaas, chave primária — globalmente único no Asaas, mesmo padrão de unicidade já usado para `cpfCnpj` do associado), `franquiaId` (FK, no `ESCOPO_DIRETO` da extension de multi-franquia — `req.prisma.pagamentoAsaas` já vem automaticamente filtrado/injetado pela franquia da requisição), `customerId`, `cpfCnpj`/`nome` (**resolvidos e cacheados aqui** — é o que elimina a chamada a `GET /v3/customers/{id}` a cada leitura), `value`/`dueDate`/`paymentDate`/`status`/`description` (mesmos campos que o Asaas já devolvia), `atualizadoEm`. Índices em `(franquiaId, dueDate)` e `(franquiaId, status)` — as duas formas como a tabela é consultada. `dueDate`/`paymentDate` continuam como `String` (não `DateTime`), a mesma convenção do resto do projeto (`classificarPagamento` etc.), pra evitar bug de fuso horário.

**Desde o AJUSTE 15** (ver seção própria mais abaixo): `dateCreated` (`String?`, nullable — campo "data de emissão" do Asaas, mesmo formato `YYYY-MM-DD` de `dueDate`/`paymentDate`; nullable porque linhas gravadas **antes** da migração `20260911145653_add_date_created_pagamentos_asaas` não têm esse valor até passarem de novo pelo webhook ou por uma reconciliação/backfill) + índices `(franquiaId, dateCreated)` e `(franquiaId, paymentDate)` (o segundo já era consultado, mas sem índice dedicado até este ajuste, por causa do novo `filtro_periodo=pagamento`). Populado nos 3 caminhos que já escrevem nesta tabela (webhook, backfill, reconciliação) via a mesma `upsertPagamento` (`pagamentosAsaas.service.js`) — nenhuma lógica duplicada.

**Webhook — `POST /api/asaas/webhook/:franquiaId`** (`asaasWebhook.controller.js`, sem `auth`/`escopoFranquia` — o Asaas não manda `Authorization: Bearer`): a franquia vem da própria URL, e a autenticação é por um token secreto no header `asaas-access-token` (campo nativo "Token de acesso" do cadastro de webhook do Asaas — não HMAC de assinatura). O token é gerado por franquia em `POST /api/config/asaas-webhook/gerar` (mostrado só uma vez, igual às API keys) e a URL pronta pra colar no painel do Asaas em `GET /api/config/asaas-webhook` (usa `PUBLIC_BASE_URL`, se configurada, pra montar a URL absoluta).

- **Eventos tratados como upsert**: `PAYMENT_CREATED`, `PAYMENT_UPDATED`, `PAYMENT_CONFIRMED`, `PAYMENT_RECEIVED`, `PAYMENT_OVERDUE`, `PAYMENT_RESTORED`, `PAYMENT_REFUNDED`, `PAYMENT_CHARGEBACK_REQUESTED`, `PAYMENT_CHARGEBACK_DISPUTE`, `PAYMENT_AWAITING_CHARGEBACK_REVERSAL` — é sempre o **mesmo** upsert por `payment.id`, com o `status` que vier no evento; nenhum tratamento especial por tipo. `PAYMENT_DELETED` remove a linha local. Qualquer outro tipo de evento (assinaturas, transferências etc., fora do escopo desta tabela) responde `200` sem fazer nada, de propósito — pra nunca entrar na fila de retry do Asaas por um evento que nunca será processado.
- **Idempotência**: o Asaas entrega eventos "at least once" (o mesmo evento pode chegar mais de uma vez). Não existe uma tabela separada de eventos processados — a idempotência vem do próprio **upsert por `payment.id`**: cada evento do Asaas traz o estado atual e completo do pagamento (não um diff), então reprocessar o mesmo evento 2x produz exatamente o mesmo estado final, nunca duplica nem corrompe.
- **Resposta rápida**: o upsert (1 escrita local) é síncrono e acontece **antes** de responder `200`. A única parte mais pesada — resolver `cpfCnpj`/`nome` via `GET /v3/customers/{id}`, só necessária quando o pagamento é novo — roda em **segundo plano**, disparada **depois** de já ter respondido (nunca `await`ada antes da resposta): uma falha ou demora aí nunca faz o Asaas re-entregar o evento por causa disso.
- **Falhas sempre claras e logadas, nunca silenciosas**: franquia inexistente na URL → `404`; token ausente/errado/franquia sem webhook configurado ainda → `401`; payload sem `event` ou com `payment` incompleto → `400`. Todos os 4 casos logados no servidor.

**Backfill inicial — `scripts/backfill-pagamentos-asaas.js`** (dry-run por padrão, `--confirm` pra aplicar): popula a tabela local com o **histórico completo** de cada franquia (reaproveita `listarPagamentos` sem filtro de vencimento — a mesma paginação que os endpoints antigos faziam a cada leitura, só que agora **uma vez só**), usando `sincronizarJanela` (`src/services/pagamentosAsaas.service.js`) — a mesma função usada pela reconciliação e pelo endpoint HTTP, nenhuma lógica duplicada entre os 3 caminhos. `--franquia=<id>`/`--desde=`/`--ate=`/`--listar-franquias`, mesmas convenções dos outros scripts do projeto. Necessário rodar **uma vez**, depois do deploy, pra trazer o histórico que existia antes do primeiro webhook — sem ele a tela fica vazia/incompleta até os eventos novos irem chegando aos poucos.

**Reconciliação periódica** — rede de segurança contra uma entrega de webhook perdida (o Asaas garante "at least once", não "sempre"): compara a tabela local com a API do Asaas numa janela **recente** (padrão 90 dias atrás até 30 dias à frente, `calcularJanelaReconciliacao`) e corrige divergências — cria o que falta, atualiza o que mudou e **remove** da tabela local o que não existe mais no Asaas dentro dessa janela (cobre um `PAYMENT_DELETED` perdido). Exposta em 2 caminhos, ambos convergindo em `sincronizarJanela`:
- `scripts/reconciliar-pagamentos-asaas.js` — dry-run por padrão, `--confirm`, `--dias-atras=`/`--dias-frente=`, `--franquia=`/`--listar-franquias`; pensado pra um agendador externo (cron do host, n8n) chamar periodicamente (ex.: diária) — **este projeto não tem agendador interno** (o sync do Dashboard também é disparado de fora, por um workflow n8n — "Sync Horário" —, não por código daqui; este ajuste segue o mesmo padrão em vez de introduzir uma dependência nova tipo `node-cron`).
- `POST /api/inadimplencia/reconciliar-pagamentos` (autenticado, escopado à franquia da requisição) — mesma lógica, sempre aplica (sem modo dry-run — não faz sentido num endpoint chamado por automação), janela sempre a padrão. Limpa o cache de `/resumo`/`/evolucao-mensal` ao final.

**`/resumo` e `/evolucao-mensal`**: a mudança é só na origem dos dados. `buscarPagamentosValidos` (compartilhada pelos dois endpoints) passou a consultar `req.prisma.pagamentoAsaas.findMany({ where: { dueDate: {...} } })` em vez de `listarPagamentos` (Asaas ao vivo), e resolve `associados` a partir do `cpfCnpj` **já cacheado** em cada linha local (`resolverAssociadosPorCpfCnpj`) em vez de um `mapaClientes` resolvido via `GET /v3/customers/{id}`. Como resolver esse mapa deixou de custar uma chamada de rede (é uma consulta local indexada), a otimização antiga de só resolver clientes "quando havia palavra-chave configurada" ou "só os OVERDUE" deixou de fazer sentido e foi removida — sempre resolve tudo, sem mudar nenhum resultado (resolver a mais nunca altera um cálculo, só deixava de valer a pena por causa do custo via Asaas, que não existe mais). Toda a lógica de classificação (`classificarPagamento`, faixas, críticos, `visao`, `tipo_pendencia`, exclusões, cross-references) é **exatamente a mesma de antes** — ver seção "Teste realizado" abaixo para a confirmação numérica.

**Teste realizado** (Postgres real, servidor Express real, mock multi-tenant do Asaas — `backend/test-ajuste14.js`, 89/89 asserções, ver "Testes realizados" no fim deste README para o log completo):

- Webhook: `PAYMENT_CREATED` → `PAYMENT_OVERDUE` → `PAYMENT_RECEIVED` → `PAYMENT_UPDATED` (mudando valor **e** vencimento) — tabela local reflete corretamente cada transição, uma a uma.
- Mesmo evento entregue 2x (inclusive `PAYMENT_DELETED`) não duplica nem corrompe — sempre exatamente 1 linha por `payment.id`, valores idênticos.
- Evento fora do escopo (ex.: assinatura) → `200`, nada gravado. Franquia inexistente → `404`. Token ausente/errado → `401`.
- Backfill populando **2 franquias com datasets disjuntos** de uma vez, sem misturar dados (cada linha local com o `franquiaId` certo, nenhum vazamento entre franquias) — rodado 2x, segunda rodada com 0 criados/só atualiza (idempotente).
- Reconciliação (via endpoint HTTP) corrigindo uma divergência simulada de propósito: um `value` alterado à força direto no banco (voltou ao valor certo) e uma linha "órfã" inserida à mão simulando um `PAYMENT_DELETED` perdido (removida).
- **Regressão — mesmos números de antes**: o dataset e as asserções já validadas em `test-status-ajustes.js` (17 pagamentos, 4 clientes) foram reproduzidos **verbatim** contra o caminho novo (Postgres local, populado via backfill) — `valor_total_faturado`, `valor_inadimplente`/`valor_adimplente`, as 7 faixas nos dois modos de `visao`, `valor_criticos`, `tipo_pendencia`, tolerância e exclusão por CPF/CNPJ deram **exatamente os mesmos valores** da rodada anterior (ex.: `valor_total_faturado: 3500`, `valor_inadimplente: 1700` no Grupo A; `faixas.ate_vencimento: 999` no Grupo B em `historico`) — confirma que a troca de fonte de dados não mudou nenhum resultado.
- **Tempo de resposta**: medido `/resumo` via Postgres local (`forcar=true`, 5 chamadas) contra uma chamada equivalente direto ao mock do Asaas (`/payments` + 2x `/customers/{id}`, 5 chamadas). Com o mock rodando local (latência de rede ~0), os dois números ficaram na mesma ordem de grandeza (poucos milissegundos) — o que esta medição prova é a mudança **estrutural** (de N chamadas HTTP externas, paginadas, por leitura, para 1 consulta indexada local), não a latência de rede real contra o Asaas em produção, que este mock não reproduz; o ganho real esperado em produção é proporcional ao histórico da franquia e à latência real da API do Asaas, tipicamente bem maior que o medido aqui.

**UI em Configurações — card "Webhook do Asaas"** (`frontend/app/configuracoes/page.js`): descuido do brief original — os 2 endpoints acima (`GET`/`POST /api/config/asaas-webhook`) existiam só via `curl`. Adicionado no mesmo padrão visual/de fluxo dos cards "Chaves de API" e "Webhook de Cadastro/Faturamento" já existentes na tela: `webhook_url` sempre visível (não é segredo, com botão "Copiar"); botão "Gerar token"/"Gerar novo token" com confirmação prévia (avisando que regenerar invalida o token atual imediatamente) e revelação do token completo **uma única vez**, em modal com botão copiar — mesmo padrão de `criarApiKey`; quando já existe um token gerado, mostra o token mascarado **e** "Token configurado — gerado em DD/MM/AAAA" (em vez de campo vazio, que passaria a impressão de nada configurado) — data vinda do `gerado_em` novo na resposta de ambos os endpoints (ver abaixo), não de uma coluna nova: reaproveita o `atualizado_em`/`@updatedAt` que a linha em `configuracoes` já tinha. Texto de apoio no card e no modal deixa explícito que o token vai no campo "Token de autenticação" do cadastro do webhook no painel do Asaas — **não** no botão "Gerar Token" nativo do Asaas (token diferente, não autentica com este backend).

  - `GET /api/config/asaas-webhook` e `POST /api/config/asaas-webhook/gerar` passaram a devolver também `gerado_em` (ISO; `null` quando `configurado: false`). `setAsaasWebhookToken` (`config.service.js`) devolve agora `{ valor, atualizadoEm }` do próprio registro upsertado, em vez de só o valor — o `gerado_em` da resposta do `POST` é assim garantidamente **idêntico** ao que um `GET` logo em seguida devolve, sem depender de dois relógios (`new Date()` no controller vs. o `atualizado_em` gravado pelo Prisma) que poderiam divergir por alguns milissegundos.
  - Testado à parte (Postgres real + servidor real + chamadas HTTP reais, mesmo padrão do resto do projeto): `gerado_em: null`/`configurado: false` antes de qualquer token; após gerar, `configurado: true` e `gerado_em` do `GET` batendo exatamente com o do `POST`; ao regenerar, novo token (diferente do anterior), `gerado_em` avança e o token mascarado do `GET` reflete o token novo, não o antigo. Rodada completa de `test-ajuste14.js` (89/89) confirmada sem regressão depois da mudança em `setAsaasWebhookToken`.

**Fora do código — passos manuais pós-deploy (PENDENTES, nenhum feito ainda)**: depois do deploy, por franquia:

1. Gerar o token (agora pela própria tela, em Configurações → "Webhook do Asaas" → "Gerar token" — ou, se preferir, `POST /api/config/asaas-webhook/gerar` via `curl`) e colar a `webhook_url`/token retornados no cadastro de webhook do painel do Asaas (campo "Token de acesso"), selecionando os eventos listados acima.
2. Rodar `node scripts/backfill-pagamentos-asaas.js --confirm` (ou por franquia, `--franquia=<id>`) pra trazer o histórico.
3. **Agendar a reconciliação periódica.** O script (`scripts/reconciliar-pagamentos-asaas.js`) e o endpoint (`POST /api/inadimplencia/reconciliar-pagamentos`) existem e estão testados, mas **nenhum dos dois roda sozinho** — não há agendador interno neste projeto (ver docblock do script). É preciso criar um **workflow novo no n8n** (ainda não existe), no mesmo padrão do "Sync Horário" já usado pelo Dashboard, disparando `POST /api/inadimplencia/reconciliar-pagamentos` periodicamente (ex.: diária) para cada franquia. Sem esse passo, a tabela local só se mantém correta enquanto o webhook não perder nenhuma entrega — a reconciliação é a rede de segurança pra quando isso acontecer, e hoje ela só roda se alguém disparar manualmente.

### AJUSTE 15 — Repaginar filtros da Taxa de Inadimplência

**Objetivo**: reorganização da barra de filtros da tela "Taxa de Inadimplência" (UI + alguns filtros novos/expandidos), mantendo **intacta** a lógica de cálculo por trás — `classificarPagamento`, `computarFaixasECriticos`, a semântica de `visao`, tolerância e exclusões não mudaram nenhuma linha; os filtros novos só restringem/recompõem **qual conjunto de pagamentos** chega até essas funções, do mesmo jeito que `renegociacao`/`em_juridico`/`bloqueado` já faziam antes deste ajuste.

**1. `filtro_periodo` (`vencimento`\|`emissao`\|`pagamento`, padrão `vencimento`)** — decide **qual campo de data** o par `venc_de`/`venc_ate` filtra, em vez de sempre `dueDate`:

| `filtro_periodo` | Campo usado | Observação |
|---|---|---|
| `vencimento` (padrão) | `dueDate` | Comportamento idêntico a antes deste ajuste — zero regressão quando o parâmetro não é enviado. |
| `emissao` | `dateCreated` | Data em que a cobrança foi criada no Asaas (ver campo novo na tabela `pagamentos_asaas`, seção acima). |
| `pagamento` | `paymentDate` | Cobranças **sem** `paymentDate` (ainda não pagas) ficam **de fora** quando este modo está ativo, mesmo com uma janela `venc_de`/`venc_ate` bem ampla — decisão de negócio do brief ("não faz sentido incluir 'não pago' filtrando por data de pagamento"), e é também o comportamento natural do Postgres: `WHERE paymentDate BETWEEN ... ` já exclui `NULL` sem precisar de um `IS NOT NULL` explícito. |

Implementado em `buscarPagamentosValidos` (`inadimplencia.controller.js`) via um mapa `CAMPO_DATA_POR_FILTRO_PERIODO` que resolve o nome da coluna Prisma a usar no `where`; **nenhuma** outra lógica do controller precisou saber qual campo está em uso. `/evolucao-mensal` bucketiza os pagamentos por mês usando **o mesmo campo** selecionado em `filtro_periodo` (antes bucketizava sempre por `dueDate.slice(0,7)`, incondicionalmente) — sem essa mudança, uma cobrança com `dueDate` fora da janela pedida mas `paymentDate`/`dateCreated` dentro dela sumiria silenciosamente do gráfico em `filtro_periodo=pagamento`/`emissao`.

**2. `situacao` (`em_aberto`\|`pagas`, múltiplo — vírgula, combinado por união/OR; ausente = sem filtro)** — filtro de **população**, diferente do antigo `tipo_pendencia` (que só reclassificava dentro de `valor_inadimplente`): `situacao` restringe o **conjunto de trabalho inteiro**, afetando também `valor_total_faturado` e todo o resto do `/resumo`/`/evolucao-mensal` computado a partir dele.

| `situacao` | Status Asaas incluídos |
|---|---|
| `em_aberto` | `OVERDUE`, `CONFIRMED`, `PENDING` |
| `pagas` | `RECEIVED`, `RECEIVED_IN_CASH` |
| `em_aberto,pagas` (os dois) | União dos dois grupos acima — **diferente** de não enviar `situacao` (sem filtro): qualquer status fora dos dois grupos (ex.: `REFUNDED`) continua de fora mesmo com os dois marcados, mas apareceria em "sem filtro". |

Validado por `validarListaMultipla` (genérico, também usado por `tipo_inadimplente` abaixo) — qualquer valor fora da lista, em qualquer posição da lista separada por vírgula, retorna `400`. Aplicado em `aplicarFiltroSituacao`, primeiro passo do pipeline de filtros do `/resumo`/`/evolucao-mensal` (antes de `aplicarFiltrosCrossReference`).

**3. `tipo_inadimplente` (`ativo`\|`juridico`\|`critico`, múltiplo — vírgula, combinado por união/OR; ausente = sem filtro)** — expande o antigo filtro `em_juridico` (tri-state exclusivo) para 3 categorias **combináveis**:

| `tipo_inadimplente` | Critério |
|---|---|
| `ativo` | `associados.em_juridico = false` |
| `juridico` | **Desde o AJUSTE 17**: existência de pelo menos 1 `CardJuridico` (qualquer etapa, inclusive "Antigos") vinculado por CPF/CNPJ — não mais `associados.em_juridico`. Ver seção "AJUSTE 17" mais abaixo para o motivo e o comportamento de divergência esperado. |
| `critico` | Associado com **pelo menos 1** cobrança com **50+ dias de atraso** (limiar baixado de 90 pra 50 no AJUSTE 17) — mesmo critério/limiar (`LIMIAR_DIAS_CRITICO = 50`) já usado por `valor_criticos` no card, **respeitando o `visao` selecionado** (`aberto`: só considera `OVERDUE`; `historico`: considera a classificação por data com tolerância, igual a `computarFaixasECriticos`). |

Combinável de verdade: um associado jurídico com uma dívida de 100 dias aparece em `tipo_inadimplente=juridico` **e** em `tipo_inadimplente=critico` ao mesmo tempo, mas enviar os dois juntos (`juridico,critico`) não duplica o valor dele na soma (é o mesmo associado, um union por CPF/CNPJ, não uma soma por combinação). O backend continua aceitando o parâmetro antigo `em_juridico` (`sim`/`nao`) para compatibilidade — não removido, só substituído na UI (ver seção de frontend abaixo).

**Pipeline de 2 passagens (evita circularidade)** — calcular "crítico" exige saber os dias de atraso de **cada** cobrança do associado, mas o filtro `tipo_inadimplente=critico` só deveria restringir o conjunto **depois** que a população já está definida (senão o próprio filtro mudaria o conjunto usado pra calcular quem é crítico, um problema circular). Ordem implementada em `resumo`/`evolucaoMensal`:

1. `aplicarFiltroSituacao` (item 2 acima) sobre os pagamentos válidos do período.
2. `aplicarFiltrosCrossReference` (`renegociacao`/`em_juridico`/`bloqueado`, como já existia).
3. `computarCpfCnpjCriticos` roda sobre esse conjunto (passo 2), **antes** do filtro de `tipo_inadimplente` ser aplicado — produz o `Set` de CPF/CNPJ "críticos".
4. `aplicarFiltroTipoInadimplente` usa esse `Set` (calculado no passo 3) pra decidir quem passa quando `critico` está entre os valores selecionados.

**Teste realizado** (Postgres real, servidor Express real, mock standalone do Asaas — `backend/test-ajuste-filtros-inadimplencia.js`, 47/47 asserções):

- `dateCreated` persistido corretamente via webhook **e** via backfill, pro mesmo pagamento.
- `filtro_periodo=emissao`/`pagamento` retornando exatamente os pagamentos esperados (incluindo a exclusão de quem nunca foi pago em `pagamento`, mesmo com janela ampla).
- `situacao=em_aberto`/`pagas`/`em_aberto,pagas` batendo com os grupos de status esperados, incluindo o caso "os dois juntos ≠ sem filtro" (status fora dos dois grupos continua de fora).
- `tipo_inadimplente=ativo`/`juridico`/`critico` (nos dois modos de `visao`) e `juridico,critico` sem duplicar valor do mesmo associado.
- `/evolucao-mensal` bucketizando por `dueDate` **e** por `paymentDate` dependendo de `filtro_periodo`, com um pagamento cujo `dueDate` e `paymentDate` caem em meses civis diferentes (fixture `p_cruza_mes`) provando que ele aparece no bucket certo em cada modo, sem sumir da janela do outro.
- Regressão: sem nenhum parâmetro novo, `valor_total_faturado`/`valor_criticos` idênticos ao comportamento anterior a este ajuste.
- `situacao`/`tipo_inadimplente` com valor inválido → `400` nos dois.
- **Nota de robustez do teste**: o dataset usa datas relativas a "hoje" (`diasAtras`/`diasAFrente`/`diaDoMes`, não datas fixas tipo `"2026-07-25"`) e as asserções de `/evolucao-mensal` calculam o total esperado de cada mês **a partir do próprio dataset** no momento da execução (não um número hardcoded) — o teste continua correto não importa em que data for rodado no futuro, sem depender de nenhuma coincidência de calendário com quando foi escrito.
- Mock do Asaas deste teste roda como **processo separado** (`mock-asaas-filtros.js`, `spawn`), não in-process no script de teste — diferente de uma primeira tentativa que rodou o mock via `http.createServer` no mesmo processo do teste e travou no passo do backfill: o backfill é chamado via `execSync` (síncrono), que bloqueia o event loop do processo do teste até o subprocesso terminar, e um mock in-process fica incapaz de responder a qualquer requisição nesse intervalo — sintoma era um timeout de 15s tentando resolver o primeiro cliente. Corrigido seguindo o mesmo padrão já usado por `mock-asaas-ajuste14.js` (mock como processo do SO à parte).
- **Regressão da suíte anterior**: rodada completa de `test-ajuste14.js` (89/89) sem nenhuma quebra depois da migração `date_created`/dos novos filtros no controller.
- `node --check` em `schema.prisma` (client gerado sem erro), `pagamentosAsaas.service.js`, `inadimplencia.controller.js`.
...
- 5 casos de **período de tolerância** (`GET`/`PATCH /api/config/tolerancia-dias`): `pay_mock_028` (1 dia em atraso, ainda não paga), `pay_mock_025` (21 dias em atraso, ainda não paga — cruza de faixa `20_30` para `0_20` com tolerância 2), `pay_mock_026` (paga com 1 dia de atraso), `pay_mock_027` (paga com exatamente 2 dias de atraso) e `pay_mock_029` (91 dias em atraso, ainda não paga — sai de `valor_criticos` com tolerância 2, sem trocar de faixa) — todos desenhados para migrar de classificação (ou de faixa, ou de `valor_criticos`) só quando a tolerância configurada é grande o suficiente para absorver o atraso.
...
### AJUSTE 9 — Nome completo do Asaas nos dados cadastrais do associado (`nome_asaas`)

Novo campo `nome_asaas` em `associados` (nullable), exposto em `GET /api/associados/:cpf_cnpj` (e em qualquer resposta que use `serializeAssociado`, ver `associados.controller.js`) e exibido no frontend no modal "Detalhe do Associado", dentro de "Dados cadastrais", como um campo **"Nome"** novo — o título do modal **continua** usando `nome` (não `nome_asaas`), nenhuma mudança nessa lógica de exibição.

**Fonte e formato**: o campo `name` retornado pela API do Asaas (`GET /v3/customers/{id}` ou busca por `cpfCnpj`), gravado **verbatim, sem nenhum parsing/tratamento** — inclui o prefixo numérico que o Asaas usa no nome (ex.: `"45.493.621 ERICA DA COSTA ROSA"`). Não tentamos extrair só o nome ou só o CPF/CNPJ dessa string; é intencional mostrar exatamente como está no Asaas.

**Por que um campo novo, e não reaproveitar `nome`**: `nome` já é usado pelo título do modal e vem do payload do n8n (tratado antes de chegar aqui), então não podia ser sobrescrito. `nome_asaas` é populado por um caminho totalmente separado (chamada direta à API do Asaas, ver abaixo), preservando os dois valores lado a lado.

**Como é populado (dois caminhos, mesma regra — nunca sobrescreve um valor já preenchido)**:

1. **Sync incremental (`POST /api/sync`, alimentado por "Sync Horário" e pelo botão "Atualizar")**: o payload do n8n **não traz** o ID do cliente no Asaas (só `cpf_cnpj`/`nome`/`telefone`/`email`/`cobrancas`) — não existia, até este ajuste, nenhuma chamada à API do Asaas dentro do fluxo de sync (`POST /api/sync` só grava o que o n8n manda; quem consulta o Asaas é o próprio n8n, por fora). Por isso foi adicionada uma nova função `buscarClientePorCpfCnpj(cpfCnpj, franquiaId)` em `asaas.service.js` (`GET /v3/customers?cpfCnpj=...`, reaproveitando o mesmo `requisitar()` interno usado por `listarPagamentos`/`obterClientesPorId`). Para cada associado do payload, **depois** do upsert normal, `POST /api/sync` chama essa função **só se `nome_asaas` ainda estiver nulo** — nunca re-busca nem sobrescreve um associado que já tem o campo preenchido. Essa é uma decisão deliberada (não a única opção): como o sync é incremental e reenvia o mesmo associado em aberto repetidamente (a cada execução, até a dívida ser resolvida), sempre re-buscar no Asaas a cada chamada custaria uma requisição extra por associado em aberto, a cada sync — desnecessário, já que o nome raramente muda. O efeito colateral aceito: se o nome mudar de verdade no Asaas depois do primeiro preenchimento, `nome_asaas` **não** se atualiza sozinho (seria preciso rodar o backfill manualmente de novo para esse associado específico, ou um `UPDATE` direto). Uma falha nessa busca (Asaas fora do ar, chave inválida, cliente não encontrado) é só logada — nunca derruba o upsert do associado/cobranças em si.
2. **Backfill único (`scripts/backfill-nome-asaas.js`)**: como a regra acima só popula `nome_asaas` para associados que passam pelo fluxo normal de sync com o campo ainda nulo, todo associado já cadastrado **antes** deste ajuste nunca ganharia o campo sozinho (eles não "chegam novos" de novo). Este script varre todos os associados com `nome_asaas` nulo (por padrão, em todas as franquias com chave do Asaas configurada; `--franquia=<id>` restringe a uma só), busca o nome de cada um no Asaas por CPF/CNPJ e preenche. Roda em modo **dry run por padrão** (só lista o que seria gravado); `--confirm` aplica de verdade. Reporta ao final quantos associados foram atualizados e lista qualquer falha (cliente removido do Asaas, documento divergente, franquia sem chave configurada, etc.) — mesma convenção de `reconciliar-cobrancas-presas.js`. Idempotente: pode ser rodado de novo a qualquer momento sem risco, já que nunca sobrescreve quem já tem o campo preenchido.

```bash
# dentro do container/ambiente com DATABASE_URL certo e a(s) chave(s) do Asaas já configuradas:
node scripts/backfill-nome-asaas.js                       # dry run — todas as franquias com chave configurada
node scripts/backfill-nome-asaas.js --confirm             # aplica de verdade
node scripts/backfill-nome-asaas.js --franquia=<id> --confirm
node scripts/backfill-nome-asaas.js --listar-franquias
```

**Migração**: `prisma/migrations/20260909120000_add_nome_asaas_associados/migration.sql` — `ALTER TABLE "associados" ADD COLUMN "nome_asaas" TEXT;` (hand-authored, ver nota sobre `prisma migrate dev` na seção de ambiente/execução).
...
2. No serviço da API, configure as variáveis de ambiente (aba *Environment*):
   ...
   - `JURIDICO_UPLOADS_DIR` → opcional, mas **recomendado definir explicitamente em produção** (ver passo 6 abaixo) — caminho onde ficam os documentos anexados no Jurídico (AJUSTE 11); se deixado em branco, usa `<raiz do backend>/uploads/juridico` dentro do próprio container
   - `JURIDICO_UPLOAD_MAX_BYTES` → opcional, padrão `20971520` (20MB) — tamanho máximo de cada documento anexado
...
6. **⚠️ Ação manual necessária (AJUSTE 11 — Documentos anexados ao Jurídico):** configure um **volume persistente** (aba *Mounts*/*Volumes* do serviço da API no EasyPanel) apontando para o mesmo caminho definido em `JURIDICO_UPLOADS_DIR` (ex.: se você definir `JURIDICO_UPLOADS_DIR=/data/juridico-uploads`, monte o volume persistente exatamente em `/data/juridico-uploads`). **Isto não é opcional**: como já dito no passo 2 e na observação no topo deste README, o filesystem do container pode não ser persistente entre deploys no EasyPanel — sem este volume, todo documento anexado por um usuário é perdido no próximo deploy da aplicação. Se preferir não mexer em `JURIDICO_UPLOADS_DIR`, monte o volume no caminho default (`/app/uploads/juridico`, já que o `WORKDIR` do Dockerfile é `/app`) em vez de definir a variável.
...
- **Cenário de cálculo** (11 pagamentos, 4 clientes no Asaas — 3 deles com associado correspondente na nossa base, 1 sem registro local — cobrindo as 6 faixas de atraso, incluindo um pagamento com exatamente 90 dias de atraso e outro com mais de 300 dias):
  ...
  - `valor_criticos` somando corretamente os pagamentos com 90+ dias, incluindo o caso de fronteira exata (90 dias) — e confirmando que esse mesmo pagamento também aparece contado na faixa `50_100` (métricas independentes, não mutuamente exclusivas).
...
- **`valor_criticos` sensível à tolerância, sem mudar de faixa**: `pay_mock_029` (91 dias em aberto) conta em `valor_criticos` (`900`) e na faixa `50_100` (`900`) com tolerância 0; com tolerância 2 (atraso efetivo de 89 dias) sai de `valor_criticos` (`0`) mas **permanece** na faixa `50_100` (`900`) — isolando o efeito da tolerância sobre o corte de 90 dias do efeito sobre a escolha de faixa, também sem `forcar=true`.
...
**AJUSTE 14 — Tabela local sincronizada via webhook do Asaas para Taxa de Inadimplência** (model novo `PagamentoAsaas`/tabela `pagamentos_asaas`, migração `20260910183034_add_pagamentos_asaas`; serviço novo `src/services/pagamentosAsaas.service.js`; endpoint novo `POST /api/asaas/webhook/:franquiaId` — sem `auth`, autenticado por `asaas-access-token`; endpoints novos `GET`/`POST /api/config/asaas-webhook`[`/gerar`]; endpoint novo `POST /api/inadimplencia/reconciliar-pagamentos`; scripts novos `scripts/backfill-pagamentos-asaas.js` e `scripts/reconciliar-pagamentos-asaas.js`; `/resumo` e `/evolucao-mensal` reescritos pra ler da tabela local — ver seção "AJUSTE 14" acima para o desenho completo) — testado contra Postgres real (serviço `postgresql` local, não embutido desta vez — ver docblock de `test-ajuste14.js`), servidor Express real, mock multi-tenant do Asaas (`mock-asaas-ajuste14.js`, 3 "contas"/tokens simultâneos, já que o app só suporta um `ASAAS_API_BASE_URL` por processo):

- **Migração**: `npx prisma migrate dev --name add_pagamentos_asaas` gerada e aplicada com sucesso sobre um banco já com as 18 migrações anteriores aplicadas (`prisma migrate deploy`) — `CREATE TABLE pagamentos_asaas` com os 2 índices (`franquia_id, due_date` / `franquia_id, status`) e a FK pra `franquias`, exatamente como desenhado no `schema.prisma`.
- **Webhook — transições de status** (franquia sintética "Multi F1", pagamento que só existe via webhook, nunca no mock): `PAYMENT_CREATED` (`PENDING`) → linha local criada com `status`/`value`/`dueDate` corretos → `PAYMENT_OVERDUE` → `status` local vira `OVERDUE` → `PAYMENT_RECEIVED` (com `paymentDate`) → `status` vira `RECEIVED`, `paymentDate` gravado → `PAYMENT_UPDATED` mudando `value` (100 → 250.5) **e** `dueDate` (2026-08-10 → 2026-08-20) numa tacada só → os dois campos atualizados corretamente na linha local. Todas as transições confirmadas consultando a tabela local direto via Prisma (não só pelo `200` da resposta).
- **Idempotência**: o mesmo evento (`PAYMENT_UPDATED` do passo anterior) entregue 2x seguidas — `200` nas duas, e depois **exatamente 1 linha** para aquele `payment.id`, com `value`/`status` continuando corretos (nunca duplicou, nunca corrompeu). `PAYMENT_DELETED` entregue 2x: primeira remove a linha, segunda também responde `200` (no-op, remover o que já não existe não é erro) e a linha continua ausente.
- **Casos de erro, sempre claros/logados**: evento fora do escopo tratado (`SUBSCRIPTION_CREATED`) → `200`, nada gravado (nunca entra em retry do Asaas por um evento que não vai ser processado mesmo). `franquiaId` inexistente na URL → `404` (logado). Token `asaas-access-token` errado → `401` (logado). Token ausente → `401` (logado). **Franquia que existe mas nunca gerou um token de webhook** (nunca chamou `POST /config/asaas-webhook/gerar` — caminho distinto dos dois anteriores, checagem `!tokenEsperado` própria no controller) → também `401` (logado), com ou sem header `asaas-access-token` enviado, nenhuma linha gravada — e confirmado que o servidor continua respondendo normalmente pra outras franquias logo em seguida (não trava nem entra em nenhum estado inconsistente por causa desse caso).
- **Backfill multi-franquia**: dry-run primeiro (`0` linhas gravadas, confirmado por contagem direta no banco) → `--confirm` populando **3 franquias de uma vez** ("Multi F1": 3 pagamentos, "Multi F2": 2, "Regressao": 17) — cada linha local com o `franquiaId` certo, **nenhuma vazou pra outra franquia**, ids continuam nos namespaces disjuntos dos respectivos datasets do mock, `cpfCnpj`/`nome` resolvidos e cacheados corretamente em cada linha. Rodado uma 2ª vez: mesma contagem total de linhas (22), `0` criados/`22` atualizados — confirma idempotência do backfill (upsert por `payment.id`, mesma base do webhook).
- **Reconciliação (endpoint HTTP)**: divergência simulada de propósito na franquia "Multi F2" — um `value` de uma linha existente alterado à força direto no banco (`5000` → `999999`) e uma linha "órfã" inserida à mão (id que não existe no mock, simulando um `PAYMENT_DELETED` perdido pelo webhook) dentro da janela padrão de reconciliação (90 dias atrás/30 à frente). `POST /api/inadimplencia/reconciliar-pagamentos` → `200`, reporta `atualizados >= 1`/`removidos >= 1`; depois da chamada, o `value` da linha corrigida voltou a `5000` e a linha órfã **não existe mais** na tabela local — confirma que a reconciliação de fato usa o Asaas como fonte de verdade pra corrigir divergência, não só pra criar o que falta.
- **Regressão — mesmos números de antes** (a parte mais importante deste ajuste): o dataset e as asserções de `test-status-ajustes.js` (17 pagamentos/4 clientes, franquia "Regressao" populada pelo mesmo backfill acima) foram reproduzidos **verbatim** contra `/resumo`/`/evolucao-mensal` lendo agora da tabela local — todos os valores batem exatamente com a rodada anterior (ao vivo): `valor_total_faturado: 3500`/`valor_inadimplente: 1700`/`valor_adimplente: 1200` (Grupo A, `tipo_pendencia`); soma mensal de `evolucao-mensal` batendo com `/resumo` (`1700`); faixas `aberto` (`ate_vencimento: 111`, `1_20: 999`, `acima_100: 444`, `valor_criticos: 444`) e `historico` (`ate_vencimento: 999`, `21_30: 505`, `31_40: 606`, `41_50: 707`) do Grupo B; `visao=aberto` (`valor_inadimplente: 1887`) x `visao=historico` (`valor_inadimplente: 3705`/`valor_adimplente: 888`); tolerância de 3 dias deslocando faixas (`ate_vencimento: 888`, `acima_100: 0`); exclusão por CPF/CNPJ do Grupo C (`excluidos.quantidade: 1`/`valor: 1500`, `valor_total_faturado: 2500`) — **zero divergência** em qualquer um desses valores.
- **Tempo de resposta**: `/resumo` via Postgres local, 5 chamadas com `forcar=true`, média ~5ms; chamada equivalente direto ao mock do Asaas (`/payments` + 2× `/customers/{id}`), 5 chamadas, média ~2ms — nesta medição local (mock sem latência de rede) os números ficaram na mesma ordem de grandeza; o que muda estruturalmente é o número de chamadas HTTP externas por leitura (várias, paginadas → zero), não capturado pela latência local do mock — ganho real esperado maior em produção, proporcional ao histórico da franquia e à latência real do Asaas.
- 89/89 asserções passaram (`test-ajuste14.js`, Postgres real via serviço `postgresql` local — não `embedded-postgres`, ver docblock do arquivo —, servidor real, mock multi-tenant real, chamadas via `fetch`).
- `node --check` em todos os arquivos backend tocados/novos deste ajuste (`schema.prisma` gera client sem erro; `prismaComEscopo.js`, `config.service.js`, `config.controller.js`, `config.routes.js`, `routes/index.js`, `pagamentosAsaas.service.js`, `asaasWebhook.controller.js`, `asaas.routes.js`, `backfill-pagamentos-asaas.js`, `reconciliar-pagamentos-asaas.js`, `inadimplencia.controller.js`, `inadimplencia.routes.js`) — 0 erros de sintaxe.

**AJUSTE 14 (complemento) — UI do webhook do Asaas em Configurações + `gerado_em`** (descuido do brief original, sinalizado pelo próprio usuário — os endpoints `GET`/`POST /api/config/asaas-webhook`[`/gerar`] só existiam via `curl`; card novo "Webhook do Asaas" em `frontend/app/configuracoes/page.js`, mesmo padrão visual/de fluxo dos cards "Chaves de API"/"Webhook de Cadastro/Faturamento" — ver seção "AJUSTE 14" acima para o desenho completo do card):

- **Backend**: `config.service.js`/`config.controller.js` passaram a devolver `gerado_em` (ISO, `null` sem token) em `GET /api/config/asaas-webhook` e em `POST /api/config/asaas-webhook/gerar`. `setAsaasWebhookToken` mudou de devolver só o valor pra devolver `{ valor, atualizadoEm }` do próprio registro upsertado (`prisma.configuracao.upsert`, sem coluna nova — reaproveita `atualizado_em`/`@updatedAt` que a linha já tinha), e o controller usa esse `atualizadoEm` direto na resposta do `POST` em vez de um `new Date()` calculado à parte — evita os dois relógios divergirem por alguns milissegundos entre a resposta do `POST` e o que um `GET` logo depois leria de volta.
- **Testado à parte** (Postgres real — banco novo `gestor_ajuste14_ui_check`, dropado ao final —, servidor Express real, chamadas via `fetch`, script descartável, sem duplicar a cobertura já existente em `test-ajuste14.js`): antes de gerar, `configurado: false`/`gerado_em: null`/`asaas_access_token: null`, `webhook_url` já preenchida mesmo assim. Após `POST /gerar`: token em texto puro (64 chars hex) só nessa resposta, `gerado_em` presente e próximo de "agora". `GET` seguinte: `configurado: true`, `gerado_em` **idêntico** (não só "próximo") ao devolvido pelo `POST`, token mascarado (nunca repete o texto puro) terminando nos mesmos 6 últimos caracteres do token real. Regeneração: novo token diferente do anterior, `gerado_em` avança (nunca fica preso na 1ª geração), `GET` seguinte reflete o token/data novos, não os antigos. 22/22 asserções passaram.
- **Regressão confirmada**: rodada completa de `test-ajuste14.js` (89/89) sem nenhuma quebra depois da mudança em `setAsaasWebhookToken` — inclusive o helper `configurarAsaas` daquele teste, que já chamava `POST /config/asaas-webhook/gerar` pra cada uma das 3 franquias sintéticas.
- `node --check` em `config.service.js`/`config.controller.js`; frontend (`lib/api.js` + `app/configuracoes/page.js`) validado com `esbuild --jsx=automatic` (sem `node_modules` disponível neste ambiente pra um `next build` completo desta vez) — 0 erros de sintaxe/JSX. Pendente rodar `npm run lint`/`next build` reais no ambiente com `node_modules` (mesmo contorno de sempre pra pasta montada do Windows) antes do deploy.

**AJUSTE 15 — Repaginar filtros da Taxa de Inadimplência** (campo novo `dateCreated` em `pagamentos_asaas`, migração `20260911145653_add_date_created_pagamentos_asaas`; filtros novos `filtro_periodo`/`situacao`/`tipo_inadimplente` em `GET /api/inadimplencia/resumo` e `/evolucao-mensal` — ver seção "AJUSTE 15" acima para o desenho completo; reorganização de UI + nenhuma reescrita da lógica de cálculo existente, conforme o brief):

- **Migração**: `npx prisma migrate dev --name add_date_created_pagamentos_asaas` gerada e aplicada sobre um banco já com as 19 migrações anteriores — `ALTER TABLE pagamentos_asaas ADD COLUMN date_created` + os 2 índices novos (`franquia_id, date_created` / `franquia_id, payment_date`), exatamente como desenhado no `schema.prisma`.
- **`dateCreated` persistido**: via webhook (`PAYMENT_CREATED` com `dateCreated` no payload) e via backfill (mesmo `upsertPagamento`) — confirmado consultando a tabela local direto via Prisma nos dois caminhos, pro mesmo `payment.id`, valor idêntico ao do payload/mock em ambos.
- **`filtro_periodo`**: `vencimento` (padrão, sem regressão — janela estreita retornando só o pagamento certo por `dueDate`), `emissao` (janela retornando só os pagamentos com `dateCreated` dentro dela) e `pagamento` (janela ampla retornando só os pagamentos com `paymentDate` preenchido dentro dela — confirmado que uma cobrança nunca paga fica de fora mesmo com a janela cobrindo seu `dueDate` de sobra).
- **`situacao`**: `em_aberto` (soma batendo com `OVERDUE+CONFIRMED+PENDING`), `pagas` (soma batendo com `RECEIVED`/variantes, cobrança `REFUNDED` do dataset ficando de fora), `em_aberto,pagas` juntos (união dos dois, **diferente** do total "sem filtro" — `REFUNDED` continua de fora mesmo com os dois marcados, confirmando que não é equivalente a "nenhum filtro"), valor inválido → `400`.
- **`tipo_inadimplente`**: `ativo` (exclui só o associado jurídico do dataset), `juridico` (só ele), `critico` em `visao=aberto` (só quem tem cobrança `OVERDUE` com 90+ dias — outro associado do dataset, com atraso grande mas só via pagamento já recebido, corretamente **não** entra nesse modo), `critico` em `visao=historico` (esse outro associado passa a entrar, via classificação por data/tolerância — mesma função já usada por `valor_criticos`), `juridico,critico` juntos no mesmo associado sem duplicar valor, valor inválido → `400`.
- **`/evolucao-mensal` bucketizando pelo campo certo**: fixture desenhada de propósito com `dueDate` num mês civil e `paymentDate` no mês civil seguinte (sempre relativa a "hoje", nunca uma data fixa — ver nota de robustez abaixo) — em `filtro_periodo=vencimento` aparece no bucket do mês do vencimento; em `filtro_periodo=pagamento`, some do bucket do mês do vencimento e aparece **inteira** no bucket do mês do pagamento, confirmando que a bucketização segue `filtro_periodo`, não sempre `dueDate` como antes deste ajuste.
- **Regressão** (a parte mais importante): sem nenhum parâmetro novo, `valor_total_faturado` = soma de todos os pagamentos do dataset e `valor_criticos` idêntico ao já validado por `computarFaixasECriticos` — confirma que nenhuma lógica de cálculo existente mudou, só a composição do conjunto de trabalho quando os filtros novos são usados.
- 47/47 asserções passaram (`test-ajuste-filtros-inadimplencia.js`, Postgres real via serviço `postgresql` local, servidor real, mock standalone do Asaas em processo separado — `mock-asaas-filtros.js`, ver nota de depuração abaixo —, chamadas via `fetch`).
- **Nota de depuração — mock precisou virar processo separado**: a 1ª versão do teste rodava o mock do Asaas in-process (`http.createServer` no mesmo script do teste) e travava com timeout de 15s no passo do backfill, tentando resolver o primeiro cliente. Causa raiz: o teste chama o script de backfill via `execSync` (síncrono), que bloqueia o event loop do processo do teste até o subprocesso terminar — um mock in-process fica incapaz de responder a qualquer requisição nesse intervalo, então o subprocesso de backfill nunca recebe resposta e estoura o timeout. Corrigido extraindo o mock pra `mock-asaas-filtros.js`, rodado via `spawn` como processo do SO à parte — mesmo padrão já usado por `mock-asaas-ajuste14.js`, que nunca teve esse problema por já seguir esse desenho.
- **Nota de robustez do teste**: a 1ª versão das asserções de `/evolucao-mensal` usava datas fixas (`"2026-07-25"`/`"2026-08-05"`) pra uma fixture "cruza mês" e números esperados hardcoded — o que ia quebrar (ou pior, passar por coincidência com um resultado errado) se este teste for rodado numa data bem diferente de quando foi escrito, e já quebrou de fato uma vez, nesta própria rodada, por coincidência de outra fixture relativa (`diasAtras(60)`) caindo no mesmo mês por acaso. Corrigido: a fixture "cruza mês" agora usa `diaDoMes(offset, dia)` (dia fixo de um mês relativo a "hoje", sempre dois meses civis adjacentes) e as asserções de `/evolucao-mensal` calculam o total esperado de cada mês **a partir do próprio array de pagamentos do teste**, não de um número fixo — o teste fica correto para qualquer data de execução, presente ou futura.
- **Regressão da suíte anterior**: rodada completa de `test-ajuste14.js` (89/89) sem nenhuma quebra depois da migração `date_created`/dos filtros novos no controller compartilhado (`inadimplencia.controller.js`).
- `node --check` em `schema.prisma` (client gerado sem erro), `pagamentosAsaas.service.js`, `inadimplencia.controller.js`, `test-ajuste-filtros-inadimplencia.js`, `mock-asaas-filtros.js` — 0 erros de sintaxe.
- **Frontend (UI da barra de filtros)**: reorganizada em 5 blocos (Período/Situação/Análise/Faixa/Tipo de inadimplente), `lib/api.js`/`app/inadimplencia/page.js`/`StatusAssociadoFilter.js`/`FaixasChart.js` atualizados, componente novo reutilizável `MultiCheckboxFilter.js` — ver README do frontend, seção "AJUSTE 15", para o desenho completo e a lista de decisões interpretativas (ex.: remoção do filtro "Tipo de pendência" da tela). Validado com `npm install`/`eslint`/`next build` reais neste ambiente (build completou a compilação e falhou só ao baixar fontes do Google Fonts, sem acesso de rede a `fonts.googleapis.com` aqui — não relacionado a este ajuste).

**AJUSTE 15 (correção pós-entrega, só frontend)**: o usuário pediu de volta a granularidade vencida/confirmada dentro de "Em aberto" (sub-filtro radio condicional, aparece só quando "Em aberto" está marcado) e a unificação dos rótulos de faixa ("Em dia"/"Acima de 100" nos dois lugares — filtro e gráfico). O sub-filtro **reaproveita o parâmetro `tipo_pendencia` já existente desde o AJUSTE 4, sem nenhuma mudança de backend** — `tipo_pendencia` já era, desde sempre, um filtro que só afeta `valor_inadimplente`/`taxa_inadimplencia_percentual` (nunca `valor_adimplente`/`valor_total_faturado`/`faixas`/`valor_criticos`, ver `STATUS_INADIMPLENTE_POR_TIPO_PENDENCIA`), então marcar "Em aberto" + "Pagas" juntos e escolher "Só vencidas" no sub-filtro já produz exatamente o comportamento pedido (só a fração "em aberto" é filtrada, "Pagas" nunca é tocado) sem tocar `inadimplencia.controller.js`. Ver README do frontend, seção "AJUSTE 15 (correção pós-entrega)", para o desenho completo da UI.

### AJUSTE 16 — Reorganizar cards da Taxa de Inadimplência + `valor_total_aberto`

**Objetivo (brief "Reorganizar cards da Taxa de Inadimplência + corrigir alinhamento do gráfico")**: reagrupar os 8 números da tela de Taxa de Inadimplência em 3 linhas (3 cards em destaque/dinheiro + 3 cards menores/taxas-contagem + 2 cards extras), e um campo novo no `/resumo`.

**Campo novo: `valor_total_aberto`** — único ponto de mudança real no backend (`exports.resumo`, `inadimplencia.controller.js`). Antes deste ajuste **não existia nenhum campo único** que respondesse "quanto falta entrar no caixa agora, incluindo o que ainda não venceu": `valor_inadimplente` chega perto, mas (a) segue `tipo_pendencia` (por padrão só `OVERDUE`+`CONFIRMED`, nunca `PENDING`), e (b) troca de critério inteiro em `visao=historico` (passa a ser por data de pagamento, não por status atual). O brief pediu explicitamente um número **sem** toggle e **sem** depender de vencimento — soma bruta de `OVERDUE`+`CONFIRMED`+`PENDING`, sempre pelo status atual do Asaas, sobre o mesmo `conjuntoTrabalho` (já passou pelos filtros de população — `situacao`/cross-reference/`tipo_inadimplente` — igual `valor_total_faturado`). Implementado reaproveitando a mesma lista de status que `STATUS_POR_SITUACAO.em_aberto` já usa pro filtro "Situação da cobrança" (mesmo conceito, dois lugares diferentes do código), sem introduzir uma constante nova. Exposto em `/resumo` como `valor_total_aberto` (arredondado, mesmo padrão dos outros valores em R$); `/evolucao-mensal` não foi tocado (o brief é só sobre os cards de `/resumo`).

**Por que não reaproveitar `valor_inadimplente` com `tipo_pendencia=todos`**: ainda ficaria faltando `PENDING` (nunca incluído nesse parâmetro, ver `STATUS_INADIMPLENTE_POR_TIPO_PENDENCIA.todos = ['OVERDUE', 'CONFIRMED']`) e o resultado mudaria de critério inteiro em `visao=historico` — o brief foi claro que "Total em Aberto" é um número fixo, "diferente do conceito de Valor Inadimplente que já existe". Confirmado por leitura de código que **nenhuma outra parte do brief precisava de mudança de backend**: "Taxa de Adimplência" já existia como campo (`taxa_adimplencia_percentual`, calculado desde antes deste ajuste a partir de `valor_adimplente`/`valor_total_faturado`) — só nunca tinha card próprio no frontend, aparecia como legenda pequena embaixo do hero "Valor adimplente"; "Total Recebido" é o mesmo `valor_adimplente` de sempre, só renomeado/promovido a card em destaque no frontend.

**Testado**: `test-ajuste-filtros-inadimplencia.js` ganhou um TESTE 7 (reaproveitando o dataset/mock/harness já existentes daquele arquivo, sem infra nova) — `valor_total_aberto` sem filtro (3700, os 4 pagamentos OVERDUE/CONFIRMED/PENDING do dataset), com `situacao=pagas` (0 — população vira só RECEIVED, confirma que ainda respeita a população filtrada e não soma o dataset inteiro por fora), e com `tipo_pendencia=vencidas&visao=historico` juntos (continua 3700 — confirma o desacoplamento de `tipo_pendencia`/`visao` pedido no brief). 50/50 asserções passaram nesse arquivo; regressão completa de `test-ajuste14.js` (89/89) sem quebra.

**Frontend**: `ResumoInadimplenciaCards.js` reescrito (3 linhas de cards: destaque/menores/extras) e `FaixasChart.js` corrigido (bug de alinhamento da última coluna, sem nenhuma mudança de cálculo) — ver README do frontend, seção "AJUSTE 16", para o desenho completo. Validado com `npm install`/`eslint`/`next build` reais neste ambiente — build completou 100% (inclusive fontes do Google Fonts, sem bloqueio de rede desta vez).

### AJUSTE 17 — Filtro Jurídico via cards reais + Críticos em 50 dias + alerta de aproximação

Brief com 3 itens independentes, todos em `inadimplencia.controller.js`.

**1. "Jurídico" (dentro de "Tipo de inadimplente") passa a consultar cards reais, não `associados.em_juridico`**: nova função `buscarCpfCnpjComCardJuridico(reqPrisma)` — `SELECT` em `cards_juridico` com `associadoId: { not: null }`, join até `associado.cpfCnpj` (o card não tem uma coluna própria de CPF/CNPJ; o caminho é card → associado → cpfCnpj), **sem filtro de etapa** — qualquer coluna conta, inclusive uma etapa "Antigos", exatamente como pedido ("não importa a etapa, só importa que existe um card"). `cardJuridico` é ESCOPO_DIRETO (franquiaId na própria tabela, injetado automaticamente pela extension do Prisma — ver `config/prismaComEscopo.js`), então nenhum `where: { franquiaId }` explícito é necessário, mesmo padrão já usado por `buscarExclusoesConfiguradas`. `aplicarFiltroTipoInadimplente` (o filtro "Tipo de inadimplente", item 6 do AJUSTE 15) trocou a condição do branch `"juridico"` de `emJuridico` (campo do associado) para `cpfCnpjComCardJuridico.has(cpfCnpj)`.

**Escopo deliberadamente limitado**: só o branch `"juridico"` mudou. O branch `"ativo"` continua `!emJuridico` (o campo, não o card) — o brief pediu a troca só no botão "Jurídico", não redefiniu "Ativo/Recuperável". Consequência documentada no código: um associado pode aparecer em "Ativo/Recuperável" (campo `em_juridico=false`) e em "Jurídico" (tem card real) **ao mesmo tempo** se os dois filtros forem marcados juntos — não é bug, é o drift que motivou a mudança, só que agora visível dos dois lados em vez de escondido atrás de um campo só. O parâmetro legado `em_juridico` (`aplicarFiltrosCrossReference`, tri-state antigo, nem enviado pela tela desde o AJUSTE 15) também não foi tocado — fora do escopo do brief.

**Divergência esperada em relação ao `em_juridico`**: o brief foi explícito que card criado manualmente (sem passar pelo toggle do Dashboard) ou card remanescente de antes do AJUSTE 10 (que sincronizava card ↔ toggle) podem fazer os números baterem diferente de antes — "não é bug, é exatamente o motivo da mudança".

**2. `LIMIAR_DIAS_CRITICO`: 90 → 50** — uma constante só, lida por `computarFaixasECriticos` (o card "Críticos") e por `computarCpfCnpjCriticos` (o filtro `tipo_inadimplente=critico`, que reaproveita a mesma constante) — nenhum "90" solto em outro lugar do arquivo, então baixar o valor bastou para cobrir "o card + qualquer lugar que reaproveite esse cálculo", como pedido. O brief só pediu pra renomear **o card** no frontend ("Críticos 50+ dias"), então a entrega original deste ajuste manteve o campo de resposta com o nome antigo `criticos_90_dias`.

**Correção pós-entrega — campo `criticos_90_dias` → `valor_criticos`**: antes de aprovar este ajuste, o usuário pediu pra também renomear o campo da API, já que `criticos_90_dias` ficou descrevendo um valor que não é mais 90. Como o ajuste ainda não tinha ido pra produção, não havia compatibilidade a preservar — o nome antigo foi trocado direto, sem alias. Escolhido `valor_criticos` (em vez de `criticos_50_dias`, a outra sugestão do próprio usuário) pra não hardcodar de novo um número que já mudou uma vez e pode mudar outra vez no futuro, seguindo o padrão de nomenclatura já usado pelos campos irmãos `valor_total_faturado`/`valor_inadimplente`/`valor_adimplente`/`valor_total_aberto`. Renomeado (campo E variável interna `criticos90Dias` → `valorCriticos`) em `inadimplencia.controller.js`, nos três arquivos de teste que leem o campo (`test-ajuste14.js`, `test-ajuste-filtros-inadimplencia.js`, `test-ajuste17-juridico-critico.js`), em `test-status-ajustes.js` (legado, não executável neste ambiente — ver nota abaixo), em `scripts/mock-asaas-server.js` (só comentário), e nos dois READMEs. No frontend, `resumo?.criticos_90_dias` virou `resumo?.valor_criticos` em `ResumoInadimplenciaCards.js` (ver README do frontend).

**Nota sobre a narrativa histórica deste README**: a primeira passada desta correção também trocou `criticos_90_dias`→`valor_criticos` nas seções que narram os ajustes anteriores a este (AJUSTE CRÍTICO 3, AJUSTE 6, e as menções embutidas a AJUSTE 5/AJUSTE CRÍTICO 2), por consistência. A pedido do usuário, essas menções dentro das seções "AJUSTE CRÍTICO 3" e "AJUSTE 6" foram revertidas de volta pra `criticos_90_dias` — são registro do que era verdade naquele momento (quando o campo genuinamente se chamava assim) e não precisam bater com o nome atual do campo. As referências dentro de "Classificação histórica de inadimplência" e "Período de tolerância" (seções de mecanismo, não de ajuste específico) permaneceram como `valor_criticos`, por descreverem o comportamento atual.

**3. Novo campo `aproximando_juridico`** — mesma base "aberto hoje" de `top_devedores`/`associados_inadimplentes` (nunca segue `visao`: é sempre snapshot do status atual do Asaas, nunca reclassificação histórica — "dias de atraso = hoje − vencimento" só faz sentido como leitura do momento presente). Agrupado por devedor como `top_devedores`, mas: (a) só entra quem tem pelo menos 1 cobrança com `diasAtraso` em `[LIMIAR_APROXIMANDO_MIN, LIMIAR_APROXIMANDO_MAX]` = `[35, 49]` (constantes novas, ambas fixas conforme o brief); (b) o `valor` somado é só das cobranças **dentro da janela** (uma cobrança do mesmo devedor já com 120 dias não entra na soma — ele já é "Crítico" por ela, não "se aproximando"); (c) `dias_atraso` exposto é o **maior** entre as cobranças que qualificaram esse devedor (pior caso, o que decide o quão perto ele está de virar Crítico). Ordenado por `dias_atraso` decrescente (desempate por `valor` decrescente) — "quem entra em Jurídico primeiro se nada for feito" no topo, como pedido. Sem corte de quantidade (diferente de `top_devedores`, que é `.slice(0, 10)`): a própria janela de 15 dias já limita o tamanho da lista, e truncar esconderia justamente quem o card existe pra mostrar.

**Testado** (`test-ajuste17-juridico-critico.js`, novo arquivo — Postgres real, servidor Express real, **sem mock do Asaas**: desde o AJUSTE 14 `/resumo` não chama mais o Asaas ao vivo, então o fixture escreve as linhas de `pagamentos_asaas` direto via Prisma, `db.pagamentoAsaas.create`, mesmo recurso já usado em `test-ajuste14.js` pra simular uma linha órfã — dispensa toda a infraestrutura de mock/backfill dos dois testes anteriores):
- Item 1: associado com `em_juridico=true` mas **sem** card real fica de fora de `tipo_inadimplente=juridico`; associado com `em_juridico=false` mas **com** card real (numa etapa "Antigos") entra; card livre (`associadoId: null`) não conta pra ninguém; `tipo_inadimplente=ativo` confirmado **inalterado** (ainda inclui o associado com card mas `em_juridico=false`, provando que "ativo" não foi tocado).
- Item 2: cobrança com 60 dias de atraso (não seria "crítica" com o limiar antigo de 90) entra em `valor_criticos` e em `tipo_inadimplente=critico`; boundary exato em 50 dias também entra (`>=`, não `>`).
- Item 3: boundary inferior (35 dias) entra, boundary logo abaixo (34) não entra, boundary logo acima (50, já "crítico") não entra; um devedor com 2 cobranças na janela (42 e 38 dias) agrega `valor` (soma) e usa o maior atraso (42) como `dias_atraso`; lista ordenada por atraso decrescente.
- 15/15 asserções passaram.

**Regressão**: `test-ajuste-filtros-inadimplencia.js` precisou de 2 ajustes por causa da mudança de comportamento deliberada — (a) seu setup ganhou uma etapa/card real do Jurídico pra `cus_c` (sem isso, o teste de `tipo_inadimplente=juridico` isolando esse associado pararia de fazer sentido sob a regra nova — cus_c ficaria de fora por falta de card, não por causa do filtro em si); (b) o valor esperado de `tipo_inadimplente=critico&visao=historico` subiu de 2700 pra 3350 (limiar 50 alcança mais um associado do dataset, cuja OUTRA cobrança — sem qualificar sozinha — entra junto por pertencer ao mesmo CPF/CNPJ já marcado crítico; ver comentário inline no teste para a conta completa). 50/50 depois do ajuste. `test-ajuste14.js` teve 1 assertion hardcoded (`valor_criticos`) atualizada de 444 pra 777 pelo mesmo motivo (limiar novo alcançando mais uma faixa do fixture) — 89/89 depois. Nenhuma das duas é uma regressão de verdade: são os próprios números mudando de propósito porque a regra de negócio mudou, exatamente como o brief pediu.

**Nota — `test-status-ajustes.js` não tocado**: este arquivo (mais antigo, usa `embedded-postgres`) já não roda neste ambiente independente deste ajuste — a dependência `embedded-postgres` não está mais no `package.json` (o projeto migrou pro padrão de Postgres real via serviço local, ver docblock de `test-ajuste14.js`), condição pré-existente e não relacionada a este brief. Ele também tem 3 asserções de `valor_criticos=444` que ficarão desatualizadas pelo mesmo motivo das outras duas (limiar 90→50) — não corrigidas aqui por não haver como executar/verificar os números novos neste ambiente; sinalizado como pendência pra quem for reativar esse arquivo no futuro.

**Frontend**: rótulo do card "Críticos 90+ dias" → "Críticos 50+ dias" (só o texto — o campo da API continua `valor_criticos`); novo componente `AproximandoJuridico.js` (mesmo estilo visual de `TopDevedores.js`, lista com barra proporcional), posicionado acima do gráfico "Valor em atraso por faixa" — ver README do frontend, seção "AJUSTE 17".

### AJUSTE 18 — Corrigir cobrança presa da Marcela + fechar a lacuna estrutural da janela

**Investigação que motivou este ajuste**: associada Marcela (CPF `27649948000129`) já tinha quitado a dívida no Asaas há dias, mas continuava aparecendo no Dashboard com "53d de atraso" — clicar "Atualizar" ou reiniciar o servidor não tinha efeito. Achados, nesta ordem:

1. `POST /api/sync` **já** implementa reconciliação por omissão (dois modos, "por_associado" e "global via `janela`" — ver seção "Reconciliação (quitação automática)" acima) — não é upsert-only, ao contrário do que se suspeitava inicialmente.
2. O "Nd de atraso" exibido é `Cobranca.diasDiferenca`, um valor **gravado** na última sincronização daquela linha (`frontend/lib/atraso.js`), não recalculado ao vivo — por isso "Atualizar"/reiniciar não muda nada visível se a cobrança específica não for tocada por nenhum sync.
3. **Causa raiz confirmada**: a `janela` que o n8n manda em `POST /api/sync` (`-53/+5 dias de hoje`) é **rolante** — só anda pra frente. Uma cobrança cujo `vencimento` envelhece além do início dela fica **permanentemente fora de alcance** de qualquer reconciliação "global" futura baseada nesse payload, mesmo que seja paga depois — o Asaas simplesmente nunca mais é consultado sobre aquele intervalo de vencimento. Era exatamente o caso da Marcela: `vencimento` da cobrança presa caindo fora da janela de qualquer sync recente.
4. Varredura sistêmica inicial (`scripts/diagnostico-cobrancas-presas-sistemico.js`, primeira versão) comparava **por associado inteiro** ("todas as linhas dele em `pagamentos_asaas` estão RECEIVED") — critério que não pegaria nem a própria Marcela em produção, porque ela também tem parcelas de renegociação genuinamente em aberto: uma linha aberta em `pagamentos_asaas` já derrubava "todasQuitadas" pro associado inteiro, escondendo a cobrança específica presa. Corrigido pra comparação **por cobrança individual** (`Cobranca.idExterno` casado contra `PagamentoAsaas.id`, a mesma cobrança no Asaas — nunca o conjunto de pagamentos do associado).

**Duas entregas, no mesmo lote:**

**1. Remediação pontual do caso já confirmado** — `--confirm` adicionado a `scripts/diagnostico-cobrancas-presas-sistemico.js`: pra cada cobrança "presa" (critério por-cobrança acima), marca `status = 'quitada'` e `quitadaEm = ` o `paymentDate` real do `pagamentos_asaas` correspondente (não a data de agora — preserva quando o pagamento de fato aconteceu). Dry-run por padrão, mesmo padrão de todos os scripts de remediação do projeto; guardrail de segurança (recusa aplicar acima de 60 presas sem `--force`, mesmo limiar de `scripts/reconciliar-cobrancas-presas.js`).

**2. Rede de segurança contínua** — a lacuna da janela rolante é estrutural, não um bug pontual: qualquer associado que pague no futuro, cujo `vencimento` já tenha envelhecido pra fora da janela do momento em que ele some do payload, ficaria preso do mesmo jeito. Fechada com uma segunda via de reconciliação, independente do payload do n8n: usa `pagamentos_asaas` (atualizado continuamente via webhook — AJUSTE 14 — sem limite de janela por data de vencimento) como fonte de verdade. Mesmo padrão já usado pro AJUSTE 14 (`reconciliar-pagamentos-asaas.js` + `POST /api/inadimplencia/reconciliar-pagamentos`):

- **Serviço compartilhado** `src/services/cobrancasPresas.service.js` — `buscarCobrancasPresas`/`aplicarQuitacao`, único lugar que implementa o critério de comparação por-cobrança; os três caminhos abaixo o reaproveitam, nenhuma lógica duplicada.
- **Script periódico** `scripts/reconciliar-cobrancas-quitadas-no-asaas.js` — pensado pra rodar via agendador externo (cron do host, ou um workflow n8n agendado, mesmo estilo do "Sync Horário"). Frequência sugerida: **diária** (só precisa pegar o que escapou, não novidade em tempo real — isso continua sendo `POST /api/sync`, horário). Dry-run por padrão, `--confirm` aplica, `--franquia=` restringe, mesmo guardrail de segurança (60/`--force`).
- **Endpoint HTTP** `POST /api/sync/reconciliar-cobrancas-quitadas` — mesma lógica, escopado à franquia da API key/sessão usada (mesmo padrão de `POST /api/sync`), sempre aplica (sem dry-run — quem chama é um agendador automatizado), responde `409` com o total encontrado se passar do guardrail (sem `--force` possível via HTTP; a saída sugerida na resposta é revisar com o script e aplicar com `--force` de lá).

**Não substitui `POST /api/sync`** — continua sendo a fonte primária de novidade (cobrança nova, valor mudado). Isto só fecha o buraco temporal que a janela rolante deixa aberto pra quitações que "envelheceram" pra fora dela.

**Testado** (`test-ajuste18-reconciliacao-cobrancas-quitadas.js`, Postgres real via serviço `postgresql` local, servidor Express real, chamadas via `fetch` e scripts via `execSync` — mesmo padrão de sempre): cenário com o caso "misto" da Marcela (cobrança velha presa + parcela de renegociação genuinamente aberta) confirmando que só a presa é tocada; controle "presa mas com vencimento dentro da janela de hoje" (prova que a aplicação não depende da janela, só o relatório diagnóstico a usa como corroboração); controle sem `id_externo` e controle com `id_externo` sem correspondência em `pagamentos_asaas` (ambos fora de escopo, reportados à parte, nunca tocados); `quitada_em` confirmado = `paymentDate` real (não "agora") em todos os casos, com um caso de borda extra (`RECEIVED` sem `paymentDate`) caindo em "agora" e sinalizado como aproximado; isolamento por franquia (script com `--franquia=` e endpoint escopado por API key não vazam pra outra franquia); idempotência (segunda rodada não encontra mais nada) nos dois scripts e no endpoint; guardrail de segurança (61 presas sintéticas) recusando aplicar sem `--force` no script E respondendo `409` no endpoint, aplicando corretamente com `--force`. **43/43 asserções passaram.**

`node --check` em todos os arquivos tocados/novos (`cobrancasPresas.service.js`, `diagnostico-cobrancas-presas-sistemico.js`, `reconciliar-cobrancas-quitadas-no-asaas.js`, `sync.controller.js`, `sync.routes.js`) — 0 erros de sintaxe. Regressão: nenhum outro arquivo de teste chama rotas de `/api/sync`, então não há suíte irmã pra rodar; `sync.controller.js`/`sync.routes.js` só ganharam código novo (export/rota adicionais), nada existente foi alterado.

### AJUSTE 19 — Nova aba "Associados" + Cadastro passa a abastecer o registro do associado

**Brief original**: hoje a tela "Cadastro" só dispara o fluxo externo (n8n → Bling/Asaas) via `POST /api/cadastros`, sem guardar nada de forma consultável no sistema. Este ajuste faz o mesmo envio também criar/atualizar o registro do `Associado` (o mesmo já usado em Taxa de Inadimplência/Jurídico/Dashboard, identificado por CPF/CNPJ), e adiciona uma aba nova pra consultar esses dados — mais importação em lote via CSV.

**1. Campos novos em `Associado`** (migração `20260924185921_add_campos_cadastro_associado` — puro `ALTER TABLE ... ADD COLUMN`, 22 colunas + 1 índice, nenhuma tabela nova): ver lista completa na seção "Modelo de dados" acima. Todos nullable, todos exclusivos do Cadastro/importação — **nunca** tocados por `POST /api/sync` nem pelo webhook/backfill do Asaas. Os campos legados `nome`/`telefone`/`email` (já existiam, escritos pelo sync) continuam intocados pelo Cadastro; os equivalentes novos (`razao_social`/`celular`/`email_cadastro`) é que passam a ser a fonte de verdade dos dados de cadastro/faturamento — deliberadamente **duplicados**, não uma migração/rename, porque os dois fluxos (sync do Asaas x Cadastro) continuam escrevendo em paralelo, sem um "dono" único do associado. Excluídos do model, conforme pedido: "Nome da pasta", "Créditos VP$", "Contratos a gerar" — esses continuam existindo só no fluxo de disparo (n8n/Google Drive, via `cadastros_enviados.nome_pasta`/`modelos_contrato_ids`), nunca viram campo do associado.

**2. `POST /api/cadastros` persiste local, sempre, antes do webhook** — ver "Fluxo, passo a passo" acima, item 1. Decisão explícita do brief: "persistir local primeiro, sempre; o disparo externo pode falhar e ser reenviado depois, mas o registro no sistema não deveria depender disso". `mapearPayloadParaAssociado` (`cadastros.controller.js`) converte as chaves em português do payload (mesmo formato já enviado ao n8n) pros campos novos do model. `valor_parcela` **não vem no payload** (é só preview calculado no frontend, `app/cadastro/page.js`, nunca enviado) — recalculado no servidor com a **mesma fórmula** já usada de verdade em `contratosGeracao.service.js` (placeholder `{{Valor da Parcela}}`): `(valor_total - valor_entrada) / numero_parcelas`, só quando `numero_parcelas > 1` e `valor_total` está preenchido — com 1 parcela só, fica `null` (a parcela é o valor total inteiro, mesma regra do preview do formulário). Em **CREATE** (CPF/CNPJ novo), como `nome`/`telefone` são `NOT NULL` no schema: `nome = razão social || nome fantasia || contato` (uma dessas sempre vem preenchida, a validação já garante razão social OU contato) e `telefone = celular || ""`. Em **UPDATE** (associado já existia, por exemplo criado antes por `POST /api/sync`), os campos legados `nome`/`telefone`/`email` **não são tocados** — só os campos novos do Cadastro são atualizados.

**Conflito de franquia**: como `Associado.cpf_cnpj` é único **globalmente** (não por franquia), a Prisma Client Extension de escopo (`prismaComEscopo.js:executarUpsertEscopado`) já rejeita com `409` qualquer tentativa de `POST /api/cadastros` com um CPF/CNPJ que já existe numa **outra** franquia — mesma proteção que já valia pra `POST /api/sync`, agora também exercitada por este endpoint.

**3. Recurso `associados` + aba nova** — ver "Restrição de telas por franquia" acima (permissão dedicada, não reaproveita `dashboard`) e a tabela de "Endpoints" acima (`GET /api/associados/registro`, `POST /api/associados/importar`[`/aplicar`]). No frontend: `AppHeader.js` ganhou o link "Associados" logo depois de "Dashboard" (posição pedida no brief); `app/associados/layout.js`+`page.js` — lista simples em linhas (Nome, CPF/CNPJ, Cidade, Telefone), busca por nome/CPF-CNPJ/e-mail, clique abre `AssociadoCadastroDetalheModal.js` (componente **novo**, somente leitura — reaproveita `GET /api/associados/:cpf_cnpj`, o mesmo endpoint do Dashboard, mas mostra um recorte diferente: os campos cadastrais do item 1, sem os controles de negociação/bloqueio/reset que continuam exclusivos do `AssociadoDetalheModal.js` do Dashboard). Confirmado com o usuário: não existe nenhum componente "estilo Bling" pra reaproveitar no sistema — a lista segue o estilo visual padrão das outras tabelas (Dashboard/Contratos).

**4. Importação em lote via CSV** — `POST /api/associados/importar` (preview, só leitura) + `POST /api/associados/importar/aplicar` (aplica de verdade), desenho em duas fases deliberadamente **sem estado no servidor**: o preview devolve os itens de `novos`/`conflitos` já totalmente populados (não só o diff), o frontend guarda isso em memória (`ImportarAssociadosModal.js`) e reenvia integralmente pra `/aplicar` — não precisa reenviar o arquivo nem depende de nenhuma sessão/cache no backend entre as duas chamadas.

- **Parser de CSV próprio** (`registroAssociados.controller.js`), sem dependência nova — não havia `csv-parse`/`papaparse` no projeto. Detecta charset (tenta UTF-8; se o resultado tiver `�`, refaz como Latin-1/CP1252 — exportações do Bling/Excel no Brasil frequentemente saem em Latin-1) e delimitador (`;` vs `,`, por frequência na 1ª linha — exports BR tipicamente usam `;`) automaticamente. Tokenizador com suporte a campos entre aspas (incluindo delimitador/quebra de linha dentro delas) e aspas escapadas (`""`).
- **Mapeamento de colunas** (cabeçalho normalizado — NFD, sem acento, minúsculo): `Nome`→`razao_social`, `Fantasia`→`nome_fantasia`, `CNPJ / CPF`→`cpf_cnpj`, `Endereço`/`Número`/`Complemento`/`Bairro`/`CEP`/`Cidade`/`UF`→campos de endereço, `Celular`/`E-mail`→`celular`/`email_cadastro`. Colunas sem correspondência (Estado civil, Profissão, Vendedor, Segmento etc.) são ignoradas, sem erro. **`Fone` é fallback de `Celular`**: só preenche `celular` quando a coluna `Celular` não existe no CSV, ou existe mas veio vazia **naquela linha específica** — `Celular`, quando presente e preenchida, sempre tem prioridade.
- **Validação de CNPJ/CPF** — só checagem de **quantidade de dígitos** (11 = CPF, 14 = CNPJ, depois de tirar tudo que não é dígito); o **valor gravado continua o original**, como digitado/exportado. **Correção pós-AJUSTE 19** (ver subseção logo abaixo): o **casamento** de cada linha com um associado já existente passou a ser por dígitos (`cpf_cnpj_digits`), não mais pelo valor exato — uma planilha com formatação diferente do que já está cadastrado cai em `conflito`, nunca em `novo`. Linha sem CNPJ/CPF ou com contagem de dígitos errada vira `erro`, nunca é adivinhada.
- **Decisão por associado, nunca em massa** — cada `conflito` (CPF/CNPJ já existente nesta franquia) precisa de uma decisão individual (`"atualizar"` ou `"pular"`) no frontend antes de `/aplicar` liberar o botão; `"novos"` são sempre criados automaticamente, sem exigir decisão (só os conflitos exigem, conforme o brief).
- Busca em lote (`findMany({ cpf_cnpj: { in: [...] } })`) pra achar os já-existentes — uma consulta só, não uma por linha.

**5. Preparo pro futuro (API de leitura externa)** — não implementado neste ajuste, só não fechada a porta: os nomes de campo dos 21 campos novos (ver "Modelo de dados" acima) são estáveis e documentados aqui de propósito, prontos pra virarem uma API de leitura pra outro sistema consumir no futuro, sem precisar renomear nada.

**Correção pós-AJUSTE 19 — CPF/CNPJ passa a ser comparado por dígitos nos 3 caminhos que escrevem `cpf_cnpj`** (`POST /api/sync`, `POST /api/cadastros`, importação de CSV). Motivada por uma limitação sinalizada no ajuste original (ver histórico do arquivo) e pedida explicitamente antes de aprovar importação de planilhas reais: o CPF/CNPJ nunca era normalizado na hora de **comparar/buscar**, então o **mesmo** associado, exportado do Bling com um formato (ex.: `"12.345.678/0001-99"`) e já existente no sistema com outro (ex.: `"12345678000199"`, vindo do Asaas), era classificado como **"novo"** em vez de **"conflito"** — arriscando duplicata em vez de atualizar o existente.

- **Campo novo `Associado.cpf_cnpj_digits`** (migração `20260924193521_add_cpf_cnpj_digits_associado` — nullable → backfill (`regexp_replace(cpf_cnpj, '\D', '', 'g')`) → `NOT NULL` → índice único, mesmo padrão já usado em `20260901130000_add_multi_franquia`): versão só-dígitos de `cpf_cnpj`, mantida em sincronia **pela aplicação** a cada escrita (nunca por trigger/generated column do banco) — é a chave de fato usada em **toda** busca/comparação de CPF/CNPJ nos 3 caminhos. `cpf_cnpj` **continua guardando o valor original, verbatim**, exatamente como sempre guardou — só pra exibição, nunca mais usado sozinho pra decidir se um associado já existe. Se a `CREATE UNIQUE INDEX` da migração falhar (`duplicate key value`), significa que já existem hoje 2+ associados com o mesmo CPF/CNPJ em formatos diferentes — o cabeçalho da própria migração traz a query SQL pra checar isso antes de aplicar.
- **`src/lib/cpfCnpj.js`** (novo, função pura `apenasDigitos`) — usado em todo lugar que precisa da versão só-dígitos.
- **Centralizado em `executarUpsertEscopado`** (`prismaComEscopo.js`) — como os 3 caminhos já usavam `associado.upsert(...)`, casar por dígitos foi corrigido **num lugar só**: o `upsert` passa a buscar o registro existente por `cpf_cnpj_digits` (não mais pelo valor exato de `cpf_cnpj`) e, ao atualizar, usa o `id` do registro encontrado (nunca o `where` original, que pode ter uma formatação diferente da já gravada). Isso corrigiu `POST /api/cadastros` **sem nenhuma mudança** em `cadastros.controller.js`. Só as buscas **auxiliares** (fora do upsert — contagem de criados/atualizados em `sync.controller.js` e o casamento em lote do preview de CSV em `registroAssociados.controller.js`) precisaram de edição direta, também trocando o casamento exato por `cpf_cnpj_digits`.
- **Raciocínio de segurança**: a alternativa (SQL cru + índice funcional do Postgres) foi descartada de propósito — Prisma Client Extensions (como o escopo por franquia) **nunca interceptam SQL cru** (`$queryRaw`), então usar SQL cru em novos pontos de busca exigiria reimplementar manualmente o isolamento por franquia em cada um deles, arriscando uma regressão no invariante mais crítico do sistema (isolamento multi-franquia). A coluna-sombra mantém toda busca como chamada Prisma normal, automaticamente coberta pela extension já existente.

**Testado** (`test-ajuste19-associados-cadastro.js` — Postgres real via serviço `postgresql` local, servidor Express real, chamadas via `fetch`, mesmo padrão de sempre): `POST /api/cadastros` persistindo o associado em CREATE (com todos os campos mapeados, incluindo `valor_parcela` calculado) mesmo com o webhook do n8n falhando (URL não configurada) — a resposta continua `201` com `status: "erro"` no cadastro, mas o associado existe no banco; `valor_parcela` ficando `null` com `numero_parcelas = 1`; UPDATE confirmando que `nome`/`telefone`/`email` legados não são tocados pelo Cadastro (só os campos novos); conflito de franquia (`409`) ao tentar `POST /api/cadastros` com CPF/CNPJ de associado de **outra** franquia; permissão `associados` (`403` sem ela, mesmo com `dashboard`, nas 3 rotas novas); detalhe `GET /api/associados/:cpf_cnpj` aceitando `dashboard` OU `associados` (e `403` sem nenhuma das duas); listagem/busca por nome, CPF/CNPJ e e-mail, com isolamento por franquia confirmado; importação CSV completa — delimitador `;` detectado, charset Latin-1 decodificado corretamente (acentuação preservada), classificação `novos`/`conflitos`/`erros`, prioridade `Celular` > `Fone`, CPF/CNPJ inválido e ausente reportados como erro; aplicar criando/atualizando/pulando conforme a decisão por linha, idempotência confirmada; preview sendo de fato somente leitura (nada é gravado antes de `/aplicar`); CSV sem coluna de CPF/CNPJ mapeável recusado com `400`. **Correção pós-AJUSTE 19** (TESTE 8b, novo): linha de CSV com CPF/CNPJ pontuado (`"999.888.777-66"`) diferente do formato já cadastrado (`"99988877766"`, sem pontuação) é classificada em `conflito` — nunca em `novo` — e o `conflito.atual` casa certo com o registro já existente via `cpf_cnpj_digits`. **79/79 asserções passaram** (75 originais + 4 da correção).

`node --check` em todos os arquivos tocados/novos deste ajuste e da correção (`cadastros.controller.js`, `associados.controller.js`, `registroAssociados.controller.js`, `associados.routes.js`, `exigirRecurso.js`, `recursos.js`, `src/lib/cpfCnpj.js`, `src/config/prismaComEscopo.js`, `sync.controller.js`) — 0 erros de sintaxe; `npx prisma validate` confirmando o schema. Regressão: as 6 outras suítes e2e do projeto que usam o serviço `postgresql` local rodadas de novo depois da correção pós-AJUSTE 19 (`test-ajuste14.js` 89/89, `test-ajuste17-juridico-critico.js` 15/15, `test-ajuste18-reconciliacao-cobrancas-quitadas.js` 43/43, `test-ajuste-filtros-inadimplencia.js` 50/50, `test-negativada-dunning.js` 25/25, `test-investigacao-faixas-juridico.js` 18/18) — todas passando, nenhuma regressão. Essas 6 suítes (mais `test-ajuste19-associados-cadastro.js`) criam associados **diretamente via Prisma cru** (`db.associado.create`, sem a extension de escopo) nos seus seeds — como `cpf_cnpj_digits` é `NOT NULL`/`UNIQUE` no banco, todos os `create` desses arquivos precisaram passar a informar o campo à mão (`apenasDigitos(cpfCnpj)`); mesma correção aplicada em `scripts/mock-asaas-server.js` (seed local de associados fictícios pro filtro "Renegociação"), que também usa o Prisma cru. `test-status-ajustes.js` não pôde ser rodado neste ambiente (depende do pacote `embedded-postgres`, não instalado aqui — gap pré-existente do ambiente; não usa o model `Associado`, então não é afetado por esta correção).

### AJUSTE 20 — Excluir cadastro (individual e em massa) na aba Associados

**Brief original**: a aba "Associados" (AJUSTE 19) precisava de um jeito de desfazer um cadastro feito por engano (ou uma importação de CSV mal feita) sem apagar o associado inteiro — ele continua existindo pro Dashboard/Jurídico/Taxa de Inadimplência, só os dados de cadastro (endereço, contato, faturamento) somem. Individual (por linha/no modal) e em massa (seleção múltipla na lista).

**1. "Excluir cadastro" = limpar (`null`) só os 22 campos do AJUSTE 19** — identificação (`tipo_pessoa`, `razao_social`, `nome_fantasia`), endereço (`cep`, `endereco`, `numero`, `complemento`, `bairro`, `cidade`, `uf`), contato (`contato_nome`, `celular`, `email_cadastro`), faturamento (`descricao_servico`, `valor_entrada`, `data_entrada`, `numero_parcelas`, `valor_parcela`, `valor_total`, `data_vencimento`, `desconto_parcela`, `observacoes_cadastro`) — reunidos numa constante só (`CAMPOS_CADASTRO`, `registroAssociados.controller.js`), reaproveitada nos 2 endpoints novos E no filtro da listagem (ver item 3). **Nunca** toca nos campos legados (`nome`, `telefone`, `email`, `em_negociacao`, `bloqueado`, `em_juridico`, `ciclo_resetado_em` etc.) nem em nenhuma outra tabela (`cobrancas`, `historico_status_associado`, `cards_juridico` ficam intocados) — sem migração nova, sem coluna nova, só um `UPDATE` dos 22 campos pra `null`.

**2. Dois endpoints, mesma limpeza, ambos exigindo `associados`** (ver "Endpoints" acima):
- `DELETE /api/associados/:cpf_cnpj/cadastro` — individual. `404` se o CPF/CNPJ não existir **nesta franquia**. Segue o mesmo padrão já usado por `atualizarBloqueio`/`resetarBloqueios` (`associados.controller.js`): `req.prisma.associado.findUnique`/`update` já são escopados por franquia pela extension (`prismaComEscopo.js`), nenhum filtro manual de franquia precisou ser escrito aqui.
- `POST /api/associados/cadastro/excluir-lote` — body `{ "cpf_cnpjs": [...] }`. Cada item processado de forma **independente** (mesmo padrão de `importarAplicar`): um CPF/CNPJ que não existe (ou é de **outra** franquia — `findUnique` escopado devolve `null` do mesmo jeito, nunca um leak) cai em `nao_encontrados` sem derrubar o resto do lote; um que já está sem cadastro é achado normalmente e conta em `excluidos` (limpar `null` pra `null` de novo é inofensivo — idempotente, não é erro). `400` se `cpf_cnpjs` vier ausente, vazio ou não for array.

**3. `GET /associados/registro` precisou de um filtro explícito novo** — investigado antes de implementar (era o ponto em aberto do brief): antes deste ajuste, a listagem da aba "Associados" não filtrava por cadastro nenhum, mostrava **toda** a carteira da franquia, inclusive associado que nunca passou por Cadastro/importação (só existe via sync do Asaas). Sem um filtro explícito, "excluir cadastro" limparia os campos mas o associado continuaria aparecendo na aba — o oposto do esperado, já que a aba existe justamente pra mostrar quem **tem** dado de cadastro. Solução: `filtroTemCadastro()` (mesma constante `CAMPOS_CADASTRO` do item 1) — um `OR` verificando se pelo menos um dos 22 campos não é `null` —, combinado com o filtro de `busca` já existente via `AND` (não `OR`: buscar por um associado sem cadastro continua devolvendo vazio, não "encontra mesmo assim"). Efeito colateral esperado e correto: associados só-sync, que **já** estavam sendo listados antes (possível lacuna do AJUSTE 19, nunca reportada), agora não aparecem mais na aba "Associados" — continuam existindo normalmente em todo o resto do sistema (`GET /api/associados`, o Dashboard, não usa esse filtro).

**4. Frontend** (`app/associados/page.js`): checkbox por linha + "selecionar todos" no cabeçalho da tabela (nova coluna, só a página atual carregada — a seleção é limpa toda vez que a página ou a busca mudam, pra nunca ficar com um CPF/CNPJ selecionado que não está mais visível ou mudou de posição). Botão "Excluir N selecionados" aparece só com 1+ marcado, ao lado do botão "Importar CSV". A exclusão individual mora no modal de detalhe (`components/AssociadoCadastroDetalheModal.js`, que até este ajuste era 100% somente leitura — ganhou sua única ação de escrita, um botão "Excluir cadastro" no rodapé). As duas ações usam `window.confirm()` (mesmo padrão já usado pra excluir documento jurídico em `app/juridico/page.js`) com a mensagem pedida no brief — não o padrão "digite o nome" usado pra excluir franquia, essa ação é bem menos destrutiva (não apaga o associado, só uma parte dos dados dele). Ao confirmar (individual ou em lote), a lista recarrega — o(s) associado(s) some(m) da aba, conforme o item 3.

**Testado** (`test-ajuste20-excluir-cadastro-associados.js`, novo arquivo — mesmo padrão de sempre: Postgres real via serviço `postgresql` local, servidor Express real, chamadas via `fetch`): `GET /associados/registro` listando associado com cadastro e escondendo associado só-sync (e um segundo já-sem-cadastro, pós-exclusão) — busca por um termo que só bate no associado só-sync devolve vazio, confirmando `AND` (não `OR`) entre busca e "tem cadastro"; o associado só-sync continua aparecendo no Dashboard (`GET /api/associados`, filtro diferente, não afetado); `DELETE .../cadastro` limpando de fato os 22 campos (checados um por um) sem tocar nenhum dos 4 campos legados testados (`nome`, `telefone`, `email`, `em_negociacao`), associado sumindo de `/associados/registro` mas continuando no Dashboard; `403` sem o recurso `associados` (mesmo com `dashboard`); `404` pra CPF/CNPJ inexistente E pra CPF/CNPJ de outra franquia (confirmado que o registro da outra franquia fica intocado — nunca um leak nem um 403 que revelasse a existência); `POST .../excluir-lote` com uma mistura de 4 CPF/CNPJ (com cadastro, já sem cadastro, inexistente, de outra franquia) confirmando `excluidos: 2`/`nao_encontrados` com os 2 certos, e que nenhum dos 2 problemáticos derrubou os outros 2; `403` sem `associados` e `400` com lista ausente/vazia/não-array; e um lote inteiro (franquia B) tentando excluir um CPF/CNPJ da franquia A confirmando `excluidos: 0`/isolamento total. **59/59 asserções passaram.**

`node --check` em `registroAssociados.controller.js`/`associados.routes.js`/`test-ajuste20-excluir-cadastro-associados.js` — 0 erros de sintaxe. Regressão: `test-ajuste19-associados-cadastro.js` rodado de novo (79/79 — precisou de 1 ajuste no próprio fixture do teste de isolamento por franquia, que criava um associado só com campos legados: sem nenhum campo de cadastro ele deixou de aparecer na listagem por causa do filtro novo do item 3, o que não tinha nada a ver com o que aquele teste queria validar — adicionada uma `razao_social` mínima ao fixture) e as 6 suítes e2e restantes do projeto (`test-ajuste14.js` 89/89, `test-ajuste17-juridico-critico.js` 15/15, `test-ajuste18-reconciliacao-cobrancas-quitadas.js` 43/43, `test-ajuste-filtros-inadimplencia.js` 50/50, `test-negativada-dunning.js` 25/25, `test-investigacao-faixas-juridico.js` 18/18) — nenhuma toca `/associados/registro` ou as rotas novas, todas passando, nenhuma regressão.

**Frontend**: `lib/api.js` (`excluirCadastroAssociado`, `excluirCadastroAssociadosLote`), `app/associados/page.js` (checkboxes + "Excluir N selecionados"), `components/AssociadoCadastroDetalheModal.js` (botão "Excluir cadastro" no rodapé) — validado com `eslint` e `next build` reais neste ambiente, build completo sem erros.

**Frontend**: `lib/api.js` (`getAssociadosRegistro`, `previewImportacaoAssociados`, `aplicarImportacaoAssociados`), `lib/auth.js` (`associados` incluído em `temRecurso`/`ORDEM_PRIORIDADE_ROTAS`, logo depois de `dashboard`), `lib/recursos.js` (checkbox novo em Controle Geral), `components/icons.js` (`IconUpload`), `components/AssociadoCadastroDetalheModal.js` e `components/ImportarAssociadosModal.js` (novos), `app/associados/layout.js`+`page.js` (novos) — validado com `npm install`/`eslint`/`next build` reais neste ambiente, build completo sem erros.
