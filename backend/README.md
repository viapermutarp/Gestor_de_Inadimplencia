...
| GET | `/api/associados/registro` | **AJUSTE 19.** Exige recurso `associados` (não `dashboard`). Listagem enxuta da carteira cadastral pra aba "Associados" — diferente de `GET /api/associados`: não filtra por cobrança em aberto, não ordena por atraso, não traz cobranças/histórico. **Paginada** (`page`, `limit`, mesmo padrão 1/100). `busca` pesquisa por nome, razão social, nome fantasia, cpf_cnpj, email OU email_cadastro (contains, case-insensitive, exceto cpf_cnpj). Resposta: `{ dados, paginacao }`. |
