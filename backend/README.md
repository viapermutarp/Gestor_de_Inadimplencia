...
| GET | `/api/franquias` | **Só SUPER_ADMIN** (403 caso contrário — ver `middleware/exigirSuperAdmin.js`). Lista todas as franquias (ativas e inativas), mais antigas primeiro, cada uma já com o próprio usuário titular embutido: `[{ id, nome, ativo, criado_em, recursos_permitidos, usuario: { id, nome, email, ativo, ultimo_login_em } \| null }]`. `usuario: null` só acontece pra franquias que nunca passaram por `POST /api/franquias` (hoje, na prática, só a franquia semeada pela migração da Fase 1). |
| POST | `/api/franquias` | **Só SUPER_ADMIN.** Body `{ "nome", "usuario": { "nome", "email", "senha" }, "recursos_permitidos"? }`. Cria a franquia E o usuário titular dela (papel `FRANQUIA`) numa única transação — não existe franquia "vazia" sem usuário neste desenho; se o usuário falhar ao criar (ex.: e-mail já em uso — `409`), a franquia também não é criada. `senha` precisa ter pelo menos 8 caracteres. `recursos_permitidos` (array de chaves de `src/config/recursos.js`) é opcional — sem ele, a franquia nasce com **todos** os recursos liberados; se informado, precisa ser só chaves válidas sem repetir (`400` caso contrário). |
| PATCH | `/api/franquias/:id` | **Só SUPER_ADMIN.** Body: qualquer subconjunto de `{ "nome", "ativo", "recursos_permitidos" }`. `ativo: false` bloqueia o login do usuário dela **imediatamente** — `POST /api/login`/`POST /api/refresh` já checam `usuario.franquia.ativo` a cada tentativa, e esta rota também revoga na hora toda sessão já aberta desse usuário (não espera o próximo refresh). Reversível a qualquer momento (`ativo: true`). `recursos_permitidos` substitui a lista inteira (não faz merge) e vale **imediatamente** na próxima requisição dessa franquia — nenhum novo login necessário (ver "Restrição de telas por franquia" abaixo). |
| PATCH | `/api/usuarios/:id` | **Só SUPER_ADMIN.** Body `{ "ativo": boolean }`. Bloqueia/desbloqueia o acesso de um usuário individual (distinto de desativar a franquia inteira — ver acima; hoje, com 1 usuário por franquia, o efeito prático é o mesmo, mas os dois controles ficam separados no banco/API de propósito). Revoga na hora as sessões já abertas do usuário quando `ativo: false`. `400` se o SUPER_ADMIN tentar bloquear a si mesmo por aqui (use `PATCH /api/perfil`). |
...
| POST | `/api/juridico/cards` | Body `{ "etapa_id", "associado_id"?, "titulo"?, "descricao"?, "responsavel"?, "prazo"? }`. Exatamente uma origem: `associado_id` (vinculado) OU `titulo` (livre) — nunca os dois, nunca nenhum (`400`). Card vinculado também não aceita `descricao` (só título/descrição livres fazem sentido pra card sem associado — mesma regra do PATCH abaixo). Nasce como último card da etapa. |
| PATCH | `/api/juridico/cards/:id` | Qualquer subconjunto de `{ "titulo", "descricao", "responsavel", "prazo" }`. Nunca muda `associado_id`/`etapa_id` (pra mover entre colunas, ver rota `.../mover`). Card vinculado a associado rejeita `titulo`/`descricao` (`400`) — não existem pra esse tipo de card. |
| PATCH | `/api/juridico/cards/:id/mover` | Body `{ "etapa_id", "indice" }`. Move o card (drag and drop) pra `etapa_id` na posição `indice` (0-based) da lista de destino — pode ser a mesma etapa, só reordenando. Reindexa `ordem` de todos os cards afetados (destino e, se mudou de etapa, também a origem). `etapa_alterada_em` só é preenchido quando a etapa muda de verdade. |
| DELETE | `/api/juridico/cards/:id` | Remove o card e reindexa `ordem` dos demais cards da etapa (fecha o buraco deixado). |

### Restrição de telas por franquia

Cada franquia passa a ter uma lista configurável (`franquias.recursos_permitidos`, array de string) de quais telas ela pode acessar — `dashboard`, `inadimplencia`, `cadastro`, `contratos`, `juridico`, `configuracoes` (chaves canônicas em `src/config/recursos.js`, reaproveitadas por `franquias.controller.js` na validação e por `middleware/exigirRecurso.js` na aplicação). **"Controle Geral" nunca entra nessa lista** — não é um recurso restringível, continua sendo SUPER_ADMIN only sem exceção, e não depende de franquia nenhuma.
