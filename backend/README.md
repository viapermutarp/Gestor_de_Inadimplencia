...
**Teste realizado** (Postgres real, servidor Express real, mock multi-tenant do Asaas — `backend/test-ajuste14.js`, 85/85 asserções, ver "Testes realizados" no fim deste README para o log completo):
...
**Fora do código — passo manual pós-deploy**: depois do deploy, é preciso, por franquia: (1) `POST /api/config/asaas-webhook/gerar` e colar a `webhook_url`/token retornados no cadastro de webhook do painel do Asaas (campo "Token de acesso"), selecionando os eventos listados acima; (2) rodar `node scripts/backfill-pagamentos-asaas.js --confirm` (ou por franquia, `--franquia=<id>`) pra trazer o histórico; (3) agendar `scripts/reconciliar-pagamentos-asaas.js` (ou o endpoint HTTP equivalente) pra rodar periodicamente (ex.: diária) num agendador externo, mesmo padrão do "Sync Horário" do Dashboard.
...
- **Casos de erro, sempre claros/logados**: evento fora do escopo tratado (`SUBSCRIPTION_CREATED`) → `200`, nada gravado (nunca entra em retry do Asaas por um evento que não vai ser processado mesmo). `franquiaId` inexistente na URL → `404` (logado). Token `asaas-access-token` errado → `401` (logado). Token ausente → `401` (logado).
...
- 85/85 asserções passaram (`test-ajuste14.js`, Postgres real via serviço `postgresql` local — não `embedded-postgres`, ver docblock do arquivo —, servidor real, mock multi-tenant real, chamadas via `fetch`).
